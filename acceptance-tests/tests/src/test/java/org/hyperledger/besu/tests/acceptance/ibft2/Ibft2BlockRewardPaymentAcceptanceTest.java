...
public class Ibft2BlockRewardPaymentAcceptanceTest extends AcceptanceTestBase {
  ...
  public void validatorsArePaidBlockReward() throws IOException {
    ...
    final BesuNode validator =
        besu.createIbft2NodeWithValidators("validator", validators);
    final BesuNode nonValidator =
        besu.createIbft2NodeWithValidators("nonValidator", validators);
    cluster.start(validator, nonValidator);
    final Account validator1Account =
        Account.create(ethTransactions, validator.getAddress());

    final int blockRewardEth = 5;
    final int blockToCheck = 2;

    cluster.verify(validator1Account.balanceAtBlockEquals(Amount.ether(0),
                                                          BigInteger.ZERO));
    cluster.verify(validator1Account.balanceAtBlockEquals(
        Amount.ether(blockRewardEth * blockToCheck),
  ...
  public void payBlockRewardToConfiguredNode() throws IOException {
    ...
    final BesuNode validator1 =
        besu.createIbft2NodeWithValidators("validator1", validators);
    ...
    final String miningBeneficiaryAddress =
        "0x1234567890123456789012345678901234567890";

    final String configWithMiningBeneficiary = initialConfig.get().replace(
        "\"ibft2\": {", "\"ibft2\": { \"miningbeneficiary\": \"" +
                            miningBeneficiaryAddress + "\",");

    validator1.setGenesisConfig(configWithMiningBeneficiary);

    final Account miningBeneficiaryAccount = Account.create(
        ethTransactions, Address.fromHexString(miningBeneficiaryAddress));
    ...
    cluster.verify(miningBeneficiaryAccount.balanceAtBlockEquals(
        Amount.ether(0), BigInteger.ZERO));
    cluster.verify(miningBeneficiaryAccount.balanceAtBlockEquals(
        Amount.ether(blockRewardEth * blockToCheck),
        BigInteger.valueOf(blockToCheck)));
  }
