...
public class PrivacyProxyTest extends AcceptanceTestBase {
  ...
  public void setUp() throws Exception {
    ...
    PrivacyGroup privacyGroup = minerNode.execute(
        contractTransactions.createSmartContract(PrivacyGroup.class));
    privacyProxy = minerNode.execute(contractTransactions.createSmartContract(
  ...
  public void rlp() throws Exception {
    ...
    contractVerifier.validTransactionReceipt(privacyProxy.getContractAddress())
        .verify(privacyProxy);
    // 0x0b0235be
    assertThat(RAW_FIRST_PARTICIPANT)
        .isEqualTo(privacyProxy.getParticipants(firstParticipant.raw())
                       .encodeFunctionCall());
    // 0xf744b089
    assertThat(RAW_ADD_PARTICIPANT)
        .isEqualTo(privacyProxy
  ...
  @Ignore(
      "return 0x which causes web3j to throw exception instead of return empty list")
  @Test
  public void
  deploysWithNoParticipant() throws Exception {
  ...
  public void canUpgrade() throws Exception {
    ...
    final PrivacyGroup upgradedContract = minerNode.execute(
        contractTransactions.createSmartContract(PrivacyGroup.class));
    ...
    assertThat(firstParticipant.raw())
        .isEqualTo(participantsAfterUpgrade.get(0));
    assertThat(secondParticipant.raw())
        .isEqualTo(participantsAfterUpgrade.get(1));
  }
