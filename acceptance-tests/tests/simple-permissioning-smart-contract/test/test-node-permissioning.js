...
contract('Permissioning: Nodes', () => {
  it('Should NOT permit any node when none have been added', async () => {
    ...
    let permitted =
        await proxy.enodeAllowed(node1High, node1Low, node1Host, node1Port);
  ...
  it('Should add a node to the whitelist and then permit that node',
     async () => {
       ...
       permitted =
           await proxy.enodeAllowed(node2High, node2Low, node2Host, node2Port);
  ...
  it('Should allow a connection between 2 added nodes', async () => {
    let permitted = await proxy.connectionAllowed(
        node1High, node1Low, node1Host, node1Port, node2High, node2Low,
        node2Host, node2Port);
    assert.equal(
        permitted,
  ...
  it('Should remove a node from the whitelist and then NOT permit that node',
     async () => {
       ...
       permitted = await proxy.connectionAllowed(node1High, node1Low, node1Host,
                                                 node1Port, node2High, node2Low,
                                                 node2Host, node2Port);
       assert.equal(
           permitted,
