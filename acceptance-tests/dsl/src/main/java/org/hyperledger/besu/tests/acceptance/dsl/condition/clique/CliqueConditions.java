...
public class CliqueConditions {
  ...
  public CliqueConditions(final EthTransactions eth,
                          final CliqueTransactions clique) {
    this.eth = eth;
    this.clique = clique;
  }
  ...
  public Condition validatorsAtBlockEqual(final String blockNumber,
                                          final BesuNode... validators) {
    return new ExpectValidatorsAtBlock(clique, blockNumber,
  ...
  public Condition validatorsAtBlockHashFromBlockNumberEqual(
    ...
    return new ExpectValidatorsAtBlockHash(clique, fromHexString(blockHash),
                                           validatorAddresses(validators));
  }
  ...
  public Condition awaitSignerSetChange(final Node node) {
    return new AwaitSignerSetChange(
        node.execute(clique.createGetSigners(LATEST)), clique);
  }

  private int cliqueBlockPeriod(final BesuNode node) {
    final String config =
        node.getGenesisConfigProvider().create(emptyList()).get();
  ...
  public static class ProposalsConfig {
    ...
    public Condition build() {
      final Map<Address, Boolean> proposalsAsAddress =
          this.proposals.entrySet().stream().collect(Collectors.toMap(
              p -> p.getKey().getAddress(), Map.Entry::getValue));
