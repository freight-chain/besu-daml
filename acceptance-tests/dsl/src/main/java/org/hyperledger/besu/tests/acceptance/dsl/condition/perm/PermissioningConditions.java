...
public class PermissioningConditions {
  ...
  public Condition addAccountsToAllowlist(final String... accounts) {
    return new AddAccountsToWhitelistSuccessfully(
        transactions.addAccountsToAllowlist(accounts));
  }
  ...
  public Condition expectAccountsAllowlist(final String... expectedAccounts) {
    return new GetExpectedAccountsWhitelist(transactions.getAccountsWhiteList(),
                                            asList(expectedAccounts));
  }

  public Condition addNodesToAllowlist(final String... nodes) {
    return addNodesToAllowlist(
        Stream.of(nodes).map(URI::create).collect(toList()));
  }
  ...
  public Condition removeNodesFromAllowlist(final String... nodes) {
    return removeNodesFromAllowlist(
        Stream.of(nodes).map(URI::create).collect(toList()));
  }
  ...
  private Condition removeNodesFromAllowlist(final List<URI> enodeList) {
    return new RemoveNodeSuccess(
        transactions.removeNodesFromAllowlist(enodeList));
  }

  public Condition getNodesAllowlist(final int expectedNodeNum) {
    return new GetNodesWhitelistPopulated(transactions.getNodesWhiteList(),
                                          expectedNodeNum);
  }

  public Condition
  expectPermissioningAllowlistFileKeyValue(final ALLOWLIST_TYPE allowlistType,
                                           final Path configFilePath,
                                           final String... val) {
    return new AllowListContainsKeyAndValue(allowlistType, asList(val),
                                            configFilePath);
  }
