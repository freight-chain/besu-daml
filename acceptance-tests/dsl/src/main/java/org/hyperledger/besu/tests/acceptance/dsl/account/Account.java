...
public class Account {
  ...
  private Account(final EthTransactions eth, final String name,
                  final Address address, final Optional<KeyPair> keyPair) {
    this.name = name;
  ...
  public static Account create(final EthTransactions eth,
  ...
  static Account fromPrivateKey(final EthTransactions eth, final String name,
  ...
  public Optional<Credentials> web3jCredentials() {
    ...
    return Optional.of(Credentials.create(privateKey.get().toString(),
                                          publicKey.get().toString()));
  }

  public Credentials web3jCredentialsOrThrow() {
    return web3jCredentials().orElseThrow(
        ()
            -> new IllegalStateException(
                "Account is missing required signing key."));
  }

  public BigInteger getNextNonce() { return BigInteger.valueOf(nonce++); }
  ...
  public Condition balanceEquals(final int expectedBalance) {
    return new ExpectAccountBalance(
        eth, this, BigDecimal.valueOf(expectedBalance), Unit.ETHER);
  }

  public Condition balanceEquals(final Amount expectedBalance) {
    return new ExpectAccountBalance(eth, this, expectedBalance.getValue(),
                                    expectedBalance.getUnit());
  }

  public Condition balanceAtBlockEquals(final Amount expectedBalance,
                                        final BigInteger block) {
    return new ExpectAccountBalanceAtBlock(eth, this, block,
  ...
  public String toString() {
    return "Account{"
        + "eth=" + eth + ", name='" + name + '\'' + ", address=" + address +
        ", nonce=" + nonce + '}';
  }
}
