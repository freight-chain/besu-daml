...
public class BesuCommand implements DefaultCommandValues, Runnable {
  ...
  public BesuCommand(final Logger logger,
  ...
  @VisibleForTesting
  protected BesuCommand(final Logger logger,
                        final RlpBlockImporter rlpBlockImporter,
                        final JsonBlockImporterFactory jsonBlockImporterFactory,
                        final RlpBlockExporterFactory rlpBlockExporterFactory,
                        final RunnerBuilder runnerBuilder,
  ...
  public BesuControllerBuilder<?> getControllerBuilder() {
    ...
    return controllerBuilderFactory
        ...
        .miningParameters(new MiningParameters(
            ...
            stratumExtranonce, Optional.empty()))
  ...
  private Optional<PermissioningConfiguration> permissioningConfiguration()
    ...
    if (localPermissionsEnabled()) {
      ...
      if (accountsPermissionsConfigFile() != null &&
          !permissionsAccountsEnabled) {
        logger.warn(
  ...
  private boolean isPruningEnabled() {
    return Optional.ofNullable(pruningOverride)
        .orElse(syncMode == SyncMode.FAST);
  }
  ...
  private Path pluginsDir() {
    if (isFullInstantiation()) {
      ...
      if (pluginsDir == null) {
        return new File(System.getProperty("besu.home", "."), "plugins")
            .toPath();
      } else {
  ...
  private File nodePrivateKeyFile() {
    final Optional<File> nodePrivateKeyFile =
        isDocker ? Optional.empty()
                 : Optional.ofNullable(standaloneCommands.nodePrivateKeyFile);
  ...
  private String rpcHttpAuthenticationCredentialsFile() {
    ...
    if (isFullInstantiation()) {
      ...
      final File authFile =
          new File(DOCKER_RPC_HTTP_AUTHENTICATION_CREDENTIALS_FILE_LOCATION);
      if (authFile.exists()) {
  ...
  private String rpcWsAuthenticationCredentialsFile() {
    ...
    if (isFullInstantiation()) {
      ...
      final File authFile =
          new File(DOCKER_RPC_WS_AUTHENTICATION_CREDENTIALS_FILE_LOCATION);
      if (authFile.exists()) {
        filename = authFile.getAbsolutePath();
      }
    }
  ...
  private File rpcHttpAuthenticationPublicKeyFile() {
    if (isDocker) {
      final File keyFile =
          new File(DOCKER_RPC_HTTP_AUTHENTICATION_PUBLIC_KEY_FILE_LOCATION);
  ...
  private File rpcWsAuthenticationPublicKeyFile() {
    if (isDocker) {
      final File keyFile =
          new File(DOCKER_RPC_WS_AUTHENTICATION_PUBLIC_KEY_FILE_LOCATION);
      return keyFile.exists() ? keyFile : null;
    } else {
  ...
  private String accountsPermissionsConfigFile() {
    return permissionsConfigFile(
        standaloneCommands.accountPermissionsConfigFile);
  }
  ...
  private String getDefaultPermissioningFilePath() {
    return dataDir().toAbsolutePath() + System.getProperty("file.separator") +
        DefaultCommandValues.PERMISSIONING_CONFIG_LOCATION;
  }

  private boolean isFullInstantiation() { return !isDocker; }
