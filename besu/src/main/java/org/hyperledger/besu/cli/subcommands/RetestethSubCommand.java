...
public class RetestethSubCommand implements Runnable {
  ...
  /**
   * Using a distinct port for retesteth will result in less testing collisions
   * and accidental RPC calls. This is <code>0xba5e</code> in hex, a hex speak
   * play on the english translation of "Besu."
   */
  ...
  private final Path dataPath =
      DefaultCommandValues.getDefaultBesuDataPath(this);
  ...
  @Option(
      ...
      description =
          "Host for Retesteth JSON-RPC HTTP to listen on (default: ${DEFAULT-VALUE})",
      arity = "1")
  ...
  @Option(
      ...
      description =
          "Port for Retesteth JSON-RPC HTTP to listen on (default: ${DEFAULT-VALUE})",
      arity = "1")
  ...
  private final JsonRPCAllowlistHostsProperty hostsAllowlist =
  ...
  public void run() {
    ...
    final RetestethConfiguration retestethConfiguration =
        new RetestethConfiguration(dataPath);
    ...
    final RetestethService retestethService = new RetestethService(
        BesuInfo.version(), retestethConfiguration, jsonRpcConfiguration);
