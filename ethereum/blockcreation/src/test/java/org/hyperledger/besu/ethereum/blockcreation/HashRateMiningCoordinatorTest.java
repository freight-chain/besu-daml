...
public class HashRateMiningCoordinatorTest {
  ...
  public HashRateMiningCoordinatorTest(final String id, final long hashRate,
  ...
  public void test() {
    ...
    assertThat(miningCoordinator.submitHashRate(id, hashRate))
        .isEqualTo(wantAdded);
    if (wantTotalHashrate == 0L) {
      assertThat(miningCoordinator.hashesPerSecond()).isEmpty();
    } else {
      assertThat(miningCoordinator.hashesPerSecond())
          .contains(wantTotalHashrate);
    }
  }
}
