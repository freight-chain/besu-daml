...
public class MockPeerDiscoveryAgent extends PeerDiscoveryAgent {
  ...
  public MockPeerDiscoveryAgent(
      final NodeKey nodeKey, final DiscoveryConfiguration config,
      final PeerPermissions peerPermissions,
      final Map<Bytes, MockPeerDiscoveryAgent> agentNetwork,
      final NatService natService) {
    super(nodeKey, config, peerPermissions, natService,
          new NoOpMetricsSystem());
    this.agentNetwork = agentNetwork;
  }

  public void processIncomingPacket(final MockPeerDiscoveryAgent fromAgent,
                                    final Packet packet) {
    ...
    handleIncomingPacket(fromAgent.getAdvertisedPeer().get().getEndpoint(),
                         packetClone);
  }
  ...
  sendOutgoingPacket(final DiscoveryPeer toPeer, final Packet packet) {
    ...
    if (!toPeer.getEndpoint().getHost().equals(
      ...
      LOG.warn(
          ...
          toPeer.getEndpoint().getHost(), agentPeer.getEndpoint().getHost());
    } else if (toPeer.getEndpoint().getUdpPort() !=
  ...
  public static class IncomingPacket {
    ...
    public IncomingPacket(final MockPeerDiscoveryAgent fromAgent,
                          final Packet packet) {
