...
public class PeerDiscoveryTableRefreshTest {
  ...
  public void tableRefreshSingleNode() {
    ...
    final PingPacketData data = PingPacketData.create(
        peers.get(0).getEndpoint(), peers.get(1).getEndpoint());
    ...
    final PongPacketData pong =
        PongPacketData.create(peers.get(0).getEndpoint(), packet.getHash());
    final Packet pongPacket =
        Packet.create(PacketType.PONG, pong, nodeKeys.get(1));
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final PingPacketData refreshData = PingPacketData.create(
          peers.get(0).getEndpoint(), peers.get(1).getEndpoint());
      final Packet refreshPacket =
          Packet.create(PacketType.PING, refreshData, nodeKeys.get(0));

      final PongPacketData refreshPong = PongPacketData.create(
          peers.get(0).getEndpoint(), refreshPacket.getHash());
      final Packet refreshPongPacket =
          Packet.create(PacketType.PONG, refreshPong, nodeKeys.get(1));
