...
public class PeerDiscoveryBondingTest {
  ...
  public void neighborsPacketNotSentUnlessBonded() {
    ...
    // Start a test peer that will send a FIND_NEIGHBORS to the agent under
    // test. It should be ignored because we haven't bonded.
    final MockPeerDiscoveryAgent otherNode = helper.startDiscoveryAgent();
    final FindNeighborsPacketData data =
        FindNeighborsPacketData.create(otherNode.getId());
    final Packet packet =
        Packet.create(PacketType.FIND_NEIGHBORS, data, otherNode.getNodeKey());
