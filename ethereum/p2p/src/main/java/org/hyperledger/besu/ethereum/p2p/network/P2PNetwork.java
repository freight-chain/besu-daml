...
public interface P2PNetwork extends Closeable {
   ...
   */
  default int getPeerCount() { return getPeers().size(); }

  /**
   * Returns a stream of peers that have been discovered on the network. These
   * peers are not necessarily connected.
   *
  ...
  /**
   * Subscribe a {@link Consumer} to all incoming {@link Message} of a given
   * sub-protocol. Calling
   * {@link #start()} on an implementation without at least having one
   * subscribed {@link Consumer} per supported sub-protocol should throw a
   * {@link RuntimeException}.
   *
   * @param capability Capability (sub-protocol) to subscribe to.
   * @param callback The callback to invoke when a message for the given
   *     capability arrives
   */
  ...
  /**
   * Adds a {@link Peer} to a list indicating efforts should be made to always
   * stay connected regardless of maxPeer limits. Non-permitted peers may be
   * added to this list, but will not actually be connected to as long as they
   * are prohibited.
   *
   * @param peer The peer that should be connected to
   * @return boolean representing whether or not the peer has been added to the
   *     list, false is returned if the peer was already on the list
   */
  boolean addMaintainConnectionPeer(final Peer peer);

  /**
   * Disconnect and remove the given {@link Peer} from the maintained peer list.
   * Peer is disconnected even if it is not in the maintained peer list. See
   * {@link #addMaintainConnectionPeer(Peer)} for details on the maintained peer
   * list.
   *
   * @param peer The peer to which connections are not longer required
   * @return boolean representing whether the peer was removed from the
   *     maintained peer list
   */
   ...
   *
   * @return true if the node is listening for network connections, false,
   *     otherwise.
   */
  boolean isListening();
   ...
   *
   * @return the enodeURL associated with this node if P2P has been enabled.
