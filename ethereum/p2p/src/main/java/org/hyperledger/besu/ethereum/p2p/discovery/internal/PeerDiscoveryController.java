 ...
 * (e.g. an expected PONG has been received after having sent a PING). This is
 * the same as having an "active" channel. <li><em>MESSAGE_EXPECTED (*)</em>: a
 * message has been sent and a response is expected. <li><em>DROPPED (*):</em>
 * the peer is no longer in our peer table.
 * </ul>
 *
...
public class PeerDiscoveryController {
  ...
  void bond(final DiscoveryPeer peer) {
    ...
    final PeerInteractionState peerInteractionState = new PeerInteractionState(
  ...
  // Load the peer first from the table, then from bonding cache or use the
  // instance that comes in.
  private DiscoveryPeer resolvePeer(final DiscoveryPeer peer) {
    final Optional<DiscoveryPeer> maybeKnownPeer = peerTable.get(peer).filter(
        known -> known.discoveryEndpointMatches(peer));
