...
/**
 * The peer discovery agent is the network component that sends and receives
 * peer discovery messages via UDP.
 */
public abstract class PeerDiscoveryAgent {
  ...
  private final List<PeerRequirement> peerRequirements =
  ...
  /* This is the {@link org.hyperledger.besu.ethereum.p2p.Peer} object
   * representing our local node. This value is empty on construction, and is
   * set after the discovery agent is started.
   */
  private Optional<DiscoveryPeer> localNode = Optional.empty();

  /* Is discovery enabled? */
  private boolean isActive = false;
  protected final Subscribers<PeerBondedObserver> peerBondedObservers =
      Subscribers.create();

  protected PeerDiscoveryAgent(final NodeKey nodeKey,
    ...
    this.bootstrapPeers = config.getBootnodes()
                              .stream()
  ...
  protected abstract CompletableFuture<InetSocketAddress>
  listenForConnections();

  protected abstract CompletableFuture<Void>
  sendOutgoingPacket(final DiscoveryPeer peer, final Packet packet);
  ...
  public CompletableFuture<Integer> start(final int tcpPort) {
    if (config.isActive()) {
      ...
      return listenForConnections().thenApply(
          (InetSocketAddress localAddress) -> {
            ...
            isActive = true;
            LOG.info("P2P peer discovery agent started and listening on {}",
                     localAddress);
  ...
  createController(final DiscoveryPeer localNode) {
  ...
  protected void handleIncomingPacket(final Endpoint sourceEndpoint,
                                      final Packet packet) {
    OptionalInt tcpPort = OptionalInt.empty();
    if (packet.getPacketData(PingPacketData.class).isPresent()) {
      final PingPacketData ping =
          packet.getPacketData(PingPacketData.class).orElseGet(null);
      if (ping != null && ping.getFrom() != null &&
          ping.getFrom().getTcpPort().isPresent()) {
   ...
   */
  protected void handleOutgoingPacket(final DiscoveryPeer peer,
                                      final Packet packet) {
    sendOutgoingPacket(peer, packet).whenComplete((res, err) -> {
      if (err != null) {
        if (err instanceof SocketException &&
            err.getMessage().contains("unreachable")) {
          LOG.debug("Peer {} is unreachable, packet: {}", peer,
                    wrapBuffer(packet.encode()), err);
        } else {
          LOG.warn("Sending to peer {} failed, packet: {}", peer,
                   wrapBuffer(packet.encode()), err);
        }
        return;
      }
  ...
  public Stream<DiscoveryPeer> streamDiscoveredPeers() {
    return controller.map(PeerDiscoveryController::streamDiscoveredPeers)
        .orElse(Stream.empty());
  }
  ...
  public Optional<DiscoveryPeer> getAdvertisedPeer() { return localNode; }

  public Bytes getId() { return id; }

  /**
   * Adds an observer that will get called when a new peer is bonded with and
   * added to the peer table.
   *
   * <p><i>No guarantees are made about the order in which observers are
   * invoked.</i>
   *
   * @param observer The observer to call.
   * @return A unique ID identifying this observer, to that it can be removed
   *     later.
   */
  ...
  private static void
  validateConfiguration(final DiscoveryConfiguration config) {
    checkArgument(config.getBindHost() != null &&
                      InetAddresses.isInetAddress(config.getBindHost()),
                  "valid bind host required");
    checkArgument(config.getAdvertisedHost() != null &&
                      InetAddresses.isInetAddress(config.getAdvertisedHost()),
                  "valid advertisement host required");
    checkArgument(config.getBindPort() == 0 ||
                      NetworkUtility.isValidPort(config.getBindPort()),
                  "valid port number required");
    checkArgument(config.getBootnodes() != null,
                  "bootstrapPeers cannot be null");
    checkArgument(config.getBucketSize() > 0,
                  "bucket size cannot be negative nor zero");
  }

  /**
   * Returns the current state of the PeerDiscoveryAgent.
   *
   * <p>If true, the node is actively listening for new connections. If false,
   * discovery has been turned off and the node is not listening for
   * connections.
   *
   * @return true, if the {@link PeerDiscoveryAgent} is active on this node,
   *     false, otherwise.
   */
