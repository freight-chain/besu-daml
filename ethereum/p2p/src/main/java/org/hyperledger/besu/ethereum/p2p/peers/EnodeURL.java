...
public class EnodeURL {
  ...
  private EnodeURL(final Bytes nodeId, final InetAddress address,
                   final OptionalInt listeningPort,
                   final OptionalInt discoveryPort) {
    checkArgument(nodeId.size() == NODE_ID_SIZE,
                  "Invalid node id.  Expected id of length: 64 bytes.");
    checkArgument(!listeningPort.isPresent() ||
                      NetworkUtility.isValidPort(listeningPort.getAsInt()),
                  "Invalid listening port.  Port should be between 1 - 65535.");
    checkArgument(!discoveryPort.isPresent() ||
                      NetworkUtility.isValidPort(discoveryPort.getAsInt()),
  ...
  public static EnodeURL fromURI(final URI uri) {
    ...
    checkStringArgumentNotEmpty(uri.getHost(),
                                "Missing or invalid ip address.");
    checkStringArgumentNotEmpty(uri.getUserInfo(), "Missing node ID.");

    checkArgument(uri.getScheme().equalsIgnoreCase("enode"),
                  "Invalid URI scheme (must equal \"enode\").");
    checkArgument(
    ...
    if (query != null) {
      final Matcher discPortMatcher =
          DISCPORT_QUERY_STRING_REGEX.matcher(query);
      if (discPortMatcher.matches()) {
        Integer discPort = Ints.tryParse(discPortMatcher.group(1));
        discoveryPort =
            discPort == null ? discoveryPort : OptionalInt.of(discPort);
      }
      checkArgument(discoveryPort.isPresent(),
                    "Invalid discovery port: '" + query + "'.");
    } else {
  ...
  private static void checkStringArgumentNotEmpty(final String argument,
                                                  final String message) {
    checkArgument(argument != null && !argument.trim().isEmpty(), message);
  }

  public static boolean sameListeningEndpoint(final EnodeURL enodeA,
  ...
  public static Bytes parseNodeId(final String nodeId) {
    ...
    checkArgument(nodeId.length() == expectedSize,
  ...
  public URI toURI() {
    final String uri =
        String.format("enode://%s@%s:%d", nodeId.toUnprefixedHexString(),
                      InetAddresses.toUriString(ip), getListeningPortOrZero());
    final OptionalInt discPort = getDiscPortQueryParam();
    if (discPort.isPresent()) {
      return URI.create(uri +
                        String.format("?discport=%d", discPort.getAsInt()));
    } else {
      return URI.create(uri);
    }
  }
  ...
  public static URI asURI(final String url) { return fromString(url).toURI(); }

  public Bytes getNodeId() { return nodeId; }

  public String getIpAsString() { return ip.getHostAddress(); }

  public InetAddress getIp() { return ip; }

  public boolean isListening() { return listeningPort.isPresent(); }

  public boolean isRunningDiscovery() { return discoveryPort.isPresent(); }

  public OptionalInt getListeningPort() { return listeningPort; }

  public int getListeningPortOrZero() { return listeningPort.orElse(0); }

  public OptionalInt getDiscoveryPort() { return discoveryPort; }

  public int getDiscoveryPortOrZero() { return discoveryPort.orElse(0); }
  ...
  public static class Builder {
    ...
    public Builder
    discoveryAndListeningPorts(final int listeningAndDiscoveryPorts) {
    ...
    /**
     * An optional listening port value. If the value is empty of equal to 0,
     * the listening port will be empty - indicating the corresponding node is
     * not listening.
     *
     * @param listeningPort If non-empty represents the port to listen on, if
     *     empty means the node is not listening
     ...
     *
     * @param listeningPort If non-zero, represents the port on which to listen
     *     for connections. A value of 0 means the node is not listening for
     *     connections.
     * @return The modified builder
     */
    ...
    /**
     * The port on which to listen for discovery messages. A value that is empty
     * or equal to 0, indicates that the node is not listening for discovery
     * messages.
     *
     * @param discoveryPort If non-empty and non-zero, represents the port on
     *     which to listen for discovery messages. Otherwise, indicates that the
     *     node is not running discovery.
     * @return The modified builder
     */
    ...
    /**
     * The port on which to listen for discovery messages. A value that is equal
     * to 0, indicates that the node is not listening for discovery messages.
     *
     * @param discoveryPort If non-zero, represents the port on which to listen
     *     for discovery messages. Otherwise, indicates that the node is not
     *     running discovery.
     * @return The modified builder
     */
