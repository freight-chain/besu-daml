...
public class BlockchainQueriesLogCacheTest {
  ...
  public static void setupClass() throws IOException {
    ...
    for (int i = 0; i < 2; i++) {
      final RandomAccessFile file = new RandomAccessFile(
          cacheDir.newFile("logBloom-" + i + ".cache"), "rws");
  ...
  private static void writeThreeEntries(final LogsBloomFilter filter,
  ...
  @Before
  public void setup() {
    final BlockHeader fakeHeader = new BlockHeader(
        Hash.EMPTY, Hash.EMPTY, Address.ZERO, Hash.EMPTY, Hash.EMPTY,
        Hash.EMPTY, testLogsBloomFilter, Difficulty.ZERO, 0, 0, 0, 0,
        Bytes.EMPTY, null, Hash.EMPTY, 0, new MainnetBlockHeaderFunctions());
    testHash = fakeHeader.getHash();
    final BlockBody fakeBody =
        new BlockBody(Collections.emptyList(), Collections.emptyList());
    when(blockchain.getBlockHashByNumber(anyLong()))
        .thenReturn(Optional.of(testHash));
    when(blockchain.getBlockHeader(any())).thenReturn(Optional.of(fakeHeader));
    when(blockchain.getBlockHeader(anyLong()))
        .thenReturn(Optional.of(fakeHeader));
  ...
  /**
   * Tests fours sets of a three block range where the seam (where the segment
   * changes) is in all possible positions in the range.
   *
   * <p>For this test both sides of the seam are cached.
   */
  @Test
  public void cachedCachedSeamTest() {
    for (long i = BLOCKS_PER_BLOOM_CACHE - 3; i <= BLOCKS_PER_BLOOM_CACHE;
         i++) {
  ...
  public void cachedUncachedSeamTest() {
    for (long i = (2 * BLOCKS_PER_BLOOM_CACHE) - 3;
         i <= 2 * BLOCKS_PER_BLOOM_CACHE; i++) {
      blockchainQueries.matchingLogs(i, i + 2, logsQuery, () -> true);
    }
  ...
  public void uncachedUncachedSeamTest() {
    for (long i = (3 * BLOCKS_PER_BLOOM_CACHE) - 3;
         i <= 3 * BLOCKS_PER_BLOOM_CACHE; i++) {
      blockchainQueries.matchingLogs(i, i + 2, logsQuery, () -> true);
    }
