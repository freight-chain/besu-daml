...
public class JsonRpcHttpServiceLoginTest {
  ...
  protected static String authPermissionsConfigFilePath =
      "JsonRpcHttpService/auth.toml";
  ...
  public static void initServerAndClient() throws Exception {
    ...
    rpcMethods = spy(new JsonRpcMethodsFactory().methods(
        CLIENT_VERSION, CHAIN_ID, genesisConfigOptions, peerDiscoveryMock,
  ...
  private static JsonRpcHttpService createJsonRpcHttpService()
    ...
    final String authTomlPath =
        Paths
            .get(ClassLoader.getSystemResource(authPermissionsConfigFilePath)
                     .toURI())
            .toAbsolutePath()
            .toString();
  ...
  public void loginWithBadCredentials() throws IOException {
    final RequestBody body = RequestBody.create(
        JSON, "{\"username\":\"user\",\"password\":\"badpass\"}");
  ...
  public void loginWithGoodCredentials() throws IOException {
    final RequestBody body = RequestBody.create(
        JSON, "{\"username\":\"user\",\"password\":\"pegasys\"}");
    final Request request =
        new Request.Builder().post(body).url(baseUrl + "/login").build();
    try (final Response resp = client.newCall(request).execute()) {
      ...
      jwtAuth.authenticate(new JsonObject().put("jwt", token), (r) -> {
        assertThat(r.succeeded()).isTrue();
        final User user = r.result();
        user.isAuthorized("noauths", (authed) -> {
  ...
  public void loginWithGoodCredentialsAndPermissions() throws IOException {
    ...
    try (final Response resp = client.newCall(request).execute()) {
      ...
      jwtAuth.authenticate(new JsonObject().put("jwt", token), (r) -> {
        ...
        user.isAuthorized("fakePermission", (authed) -> {
          assertThat(authed.succeeded()).isTrue();
  ...
  public void loginPopulatesJWTPayloadWithRequiredValues()
    ...
    try (final Response resp = client.newCall(request).execute()) {
      ...
      assertThat(jwtPayload.getJsonArray("permissions"))
          .isEqualTo(new JsonArray(list("fakePermission", "eth:blockNumber",
                                        "eth:subscribe", "web3:*")));
  ...
  private String login(final String username, final String password)
    ...
    final RequestBody loginBody =
        RequestBody.create(JSON, "{\"username\":\"" + username +
                                     "\",\"password\":\"" + password + "\"}");
    ...
    try (final Response loginResp = client.newCall(loginRequest).execute()) {
      ...
      assertThat(loginResp.body().contentType().type())
  ...
  public void checkJsonRpcMethodsAvailableWithGoodCredentialsAndPermissions()
    ...
    try (final Response resp = client.newCall(request).execute()) {
      ...
      final JsonRpcMethod netVersion =
          new NetVersion(Optional.of(BigInteger.valueOf(123)));
      final JsonRpcMethod ethBlockNumber =
      ...
      jwtAuth.authenticate(new JsonObject().put("jwt", token), (r) -> {
        ...
        Assertions
            .assertThat(AuthenticationUtils.isPermitted(
                service.authenticationService, Optional.of(user),
                ethBlockNumber))
            .isTrue();
        // eth/accounts NOT permitted
        assertThat(
            AuthenticationUtils.isPermitted(service.authenticationService,
                                            Optional.of(user), ethAccounts))
            .isFalse();
        // allowed by web3/*
        assertThat(AuthenticationUtils.isPermitted(
                       service.authenticationService, Optional.of(user),
        ...
        assertThat(
            AuthenticationUtils.isPermitted(service.authenticationService,
                                            Optional.of(user), web3Sha3))
            .isTrue();
        // NO net permissions
        assertThat(
            AuthenticationUtils.isPermitted(service.authenticationService,
                                            Optional.of(user), netVersion))
            .isFalse();
      });
    }
  }
  ...
  public void checkJsonRpcMethodsAvailableWithGoodCredentialsAndAllPermissions()
      throws IOException {
    final RequestBody body = RequestBody.create(
        JSON, "{\"username\":\"adminuser\",\"password\":\"pegasys\"}");
    final Request request =
        new Request.Builder().post(body).url(baseUrl + "/login").build();
    try (final Response resp = client.newCall(request).execute()) {
      assertThat(resp.code()).isEqualTo(200);
  ...
  public void checkPermissionsWithEmptyUser() {
    ...
    assertThat(AuthenticationUtils.isPermitted(service.authenticationService,
                                               Optional.empty(), ethAccounts))
        .isFalse();
  }
  ...
  public void web3ClientVersionUnsuccessfulWithBadBearer() throws Exception {
    ...
    try (final Response resp =
             client.newCall(buildPostRequest(body, "badtoken")).execute()) {
      assertThat(resp.code()).isEqualTo(401);
      assertThat(resp.message()).isEqualTo("Unauthorized");
    }
  }
  ...
  public void web3ClientVersionSuccessfulAfterLogin() throws Exception {
    ...
    final String id = "123";
    final RequestBody web3ClientVersionBody = RequestBody.create(
        JSON, "{\"jsonrpc\":\"2.0\",\"id\":" + Json.encode(id) +
                  ",\"method\":\"web3_clientVersion\"}");

    try (final Response web3ClientVersionResp =
             client.newCall(buildPostRequest(web3ClientVersionBody, token))
                 .execute()) {
      assertThat(web3ClientVersionResp.code()).isEqualTo(200);
      // Check general format of result
      final JsonObject json =
          new JsonObject(web3ClientVersionResp.body().string());
  ...
  public void ethSyncingUnauthorisedWithoutPermission() throws Exception {
    ...
    try (final Response resp =
             client.newCall(buildPostRequest(body, token)).execute()) {
      assertThat(resp.code()).isEqualTo(401);
      assertThat(resp.message()).isEqualTo("Unauthorized");
    }
  }
  ...
  private Request buildPostRequest(final RequestBody body,
    ...
    final Request.Builder request =
        new Request.Builder().post(body).url(baseUrl);
  ...
  private JsonObject decodeJwtPayload(final String token) {
    ...
    return new JsonObject(
        new String(Base64.getUrlDecoder().decode(payload), UTF_8));
  }
}
