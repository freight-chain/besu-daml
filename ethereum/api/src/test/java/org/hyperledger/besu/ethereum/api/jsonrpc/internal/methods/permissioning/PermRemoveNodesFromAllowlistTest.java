...
public class PermRemoveNodesFromAllowlistTest {
  ...
  @Before
  public void setUp() {
    method = new PermRemoveNodesFromAllowlist(
  ...
  public void shouldThrowInvalidJsonRpcParametersExceptionWhenBadEnode() {
    final JsonRpcRequestContext request =
        buildRequest(Lists.newArrayList(badEnode));
    final JsonRpcResponse expected =
        new JsonRpcErrorResponse(request.getRequest().getId(),
                                 JsonRpcError.NODE_ALLOWLIST_INVALID_ENTRY);

    when(nodeLocalConfigPermissioningController.removeNodes(
             eq(Lists.newArrayList(badEnode))))
  ...
  public void shouldThrowInvalidJsonRpcParametersExceptionWhenEmptyList() {
    ...
    final JsonRpcResponse expected = new JsonRpcErrorResponse(
        request.getRequest().getId(), JsonRpcError.NODE_ALLOWLIST_EMPTY_ENTRY);

    when(nodeLocalConfigPermissioningController.removeNodes(
             eq(Lists.emptyList())))
        .thenReturn(new NodesAllowlistResult(
  ...
  public void shouldRemoveSingleValidNode() {
    final JsonRpcRequestContext request =
        buildRequest(Lists.newArrayList(enode1));
    final JsonRpcResponse expected =
  ...
  public void shouldRemoveMultipleValidNodes() {
    final JsonRpcRequestContext request =
        buildRequest(Lists.newArrayList(enode1, enode2, enode3));
    final JsonRpcResponse expected =
        new JsonRpcSuccessResponse(request.getRequest().getId());
  ...
  @Test
  public void
  whenRequestContainsDuplicatedNodesShouldReturnDuplicatedEntryError() {
    final JsonRpcRequestContext request =
