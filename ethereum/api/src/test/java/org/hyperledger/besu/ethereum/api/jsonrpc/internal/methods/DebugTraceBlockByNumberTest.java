...
public class DebugTraceBlockByNumberTest {
  ...
  public void nameShouldBeDebugTraceBlockByNumber() {
    assertThat(debugTraceBlockByNumber.getName())
        .isEqualTo("debug_traceBlockByNumber");
  }
  ...
  public void shouldReturnCorrectResponse() {
    ...
    final JsonRpcRequestContext request = new JsonRpcRequestContext(
        new JsonRpcRequest("2.0", "debug_traceBlockByNumber", params));
    ...
    when(transaction2Result.getOutput())
        .thenReturn(Bytes.fromHexString("1234"));
    when(blockchain.getBlockHashByNumber(blockNumber))
        .thenReturn(Optional.of(blockHash));
    when(blockTracer.trace(eq(blockHash), any()))
    ...
    assertThat(result).usingFieldByFieldElementComparator().isEqualTo(
        DebugTraceTransactionResult.of(blockTrace.getTransactionTraces()));
  }

  @SuppressWarnings("unchecked")
  private Collection<DebugTraceTransactionResult>
  getResult(final JsonRpcSuccessResponse response) {
