...
public class PermGetNodesAllowlistTest {
  ...
  @Before
  public void setUp() {
    method = new PermGetNodesAllowlist(
  ...
  public void shouldReturnSuccessResponseWhenListPopulated() {
    ...
    final JsonRpcResponse expected =
        new JsonRpcSuccessResponse(request.getRequest().getId(),
                                   Lists.newArrayList(enode1, enode2, enode3));
    ...
    verify(nodeLocalConfigPermissioningController, times(1))
        .getNodesAllowlist();
  ...
  public void shouldReturnSuccessResponseWhenListSetAndEmpty() {
    final JsonRpcRequestContext request = buildRequest();
    final JsonRpcResponse expected = new JsonRpcSuccessResponse(
        request.getRequest().getId(), Lists.emptyList());

    when(nodeLocalConfigPermissioningController.getNodesAllowlist())
        .thenReturn(buildNodesList());
  ...
  private JsonRpcRequestContext buildRequest() {
    return new JsonRpcRequestContext(
        new JsonRpcRequest("2.0", METHOD_NAME, new Object[] {}));
  }
