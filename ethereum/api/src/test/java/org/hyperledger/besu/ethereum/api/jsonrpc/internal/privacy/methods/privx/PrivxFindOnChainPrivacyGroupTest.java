...
public class PrivxFindOnChainPrivacyGroupTest {
  ...
  private static final List<String> ADDRESSES =
      List.of("0xfe3b557e8fb62b89f4916b721be55ceb828dbd73",
  ...
  public void setUp() {
    ...
    request = new JsonRpcRequestContext(
        new JsonRpcRequest("1", "privx_findOnChainPrivacyGroup",
    ...
    privxFindOnChainPrivacyGroup = new PrivxFindOnChainPrivacyGroup(
  ...
  public void findsPrivacyGroupWithValidAddresses() {
    when(privacyController.findOnChainPrivacyGroup(ADDRESSES,
                                                   ENCLAVE_PUBLIC_KEY))
    ...
    verify(privacyController)
        .findOnChainPrivacyGroup(ADDRESSES, ENCLAVE_PUBLIC_KEY);
  }
  ...
  public void failsWithFindPrivacyGroupErrorIfEnclaveFails() {
    ...
    assertThat(response.getError())
        .isEqualTo(JsonRpcError.FIND_ONCHAIN_PRIVACY_GROUP_ERROR);
    verify(privacyController)
        .findOnChainPrivacyGroup(ADDRESSES, ENCLAVE_PUBLIC_KEY);
  }
  ...
  public void failsWithUnauthorizedErrorIfMultiTenancyValidationFails() {
    ...
    final JsonRpcResponse expectedResponse =
        new JsonRpcErrorResponse(request.getRequest().getId(),
                                 JsonRpcError.FIND_ONCHAIN_PRIVACY_GROUP_ERROR);
    final JsonRpcResponse response =
        privxFindOnChainPrivacyGroup.response(request);
