...
public class DebugTraceBlockTest {
  ...
  private final DebugTraceBlock debugTraceBlock = new DebugTraceBlock(
      () -> blockTracer, new MainnetBlockHeaderFunctions(), blockchainQueries);
  ...
  public void shouldReturnCorrectResponse() {
    final Block parentBlock = new BlockDataGenerator().block(
        BlockDataGenerator.BlockOptions.create().setBlockHeaderFunctions(
    ...
    final JsonRpcRequestContext request = new JsonRpcRequestContext(
        new JsonRpcRequest("2.0", "debug_traceBlock", params));

    final TraceFrame traceFrame = new TraceFrame(
        12, Optional.of("NONE"), Gas.of(45), Optional.of(Gas.of(56)), Gas.ZERO,
        2, Optional.empty(), null, Wei.ZERO, Bytes.EMPTY, Bytes.EMPTY,
    ...
    final TransactionProcessor.Result transaction1Result =
        mock(TransactionProcessor.Result.class);
    final TransactionProcessor.Result transaction2Result =
        mock(TransactionProcessor.Result.class);
    ...
    final BlockTrace blockTrace =
        new BlockTrace(asList(transaction1Trace, transaction2Trace));

    when(transaction1Trace.getTraceFrames())
        .thenReturn(singletonList(traceFrame));
    when(transaction2Trace.getTraceFrames())
        .thenReturn(singletonList(traceFrame));
    ...
    when(transaction1Result.getOutput())
        .thenReturn(Bytes.fromHexString("1234"));
    when(transaction2Result.getOutput())
        .thenReturn(Bytes.fromHexString("1234"));
    ...
    when(blockchainQueries.blockByHash(parentBlock.getHash()))
        .thenReturn(Optional.of(new BlockWithMetadata<>(
  ...
  public void shouldReturnErrorResponseWhenParentBlockMissing() {
    ...
    final JsonRpcErrorResponse response =
        (JsonRpcErrorResponse)debugTraceBlock.response(request);
    assertThat(response.getError())
        .isEqualByComparingTo(JsonRpcError.PARENT_BLOCK_NOT_FOUND);
  }
}
