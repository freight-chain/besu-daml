...
public final class TlsOkHttpClientBuilder {
  ...
  public TlsOkHttpClientBuilder
  withBesuCertificate(final SelfSignedP12Certificate besuCertificate) {
    this.besuCertificate = besuCertificate;
    return this;
  }

  public TlsOkHttpClientBuilder
  withOkHttpCertificate(final SelfSignedP12Certificate okHttpCertificate) {
  ...
  public OkHttpClient build() { return buildHttpClient(); }
  ...
  private OkHttpClient buildHttpClient() {
    ...
    try {
      final OkHttpClient.Builder builder =
          new OkHttpClient.Builder().sslSocketFactory(
              getCustomSslContext(trustManagerFactory, keyManagerFactory)
                  .getSocketFactory(),
              (X509TrustManager)trustManagerFactory
                  .getTrustManagers()[0]); // we only have one trust manager
      if (useHttp1) {
  ...
  private Optional<KeyManagerFactory> getKeyManagerFactory() {
    ...
    try {
      ...
      final KeyStore keyStore = KeyStore.getInstance(
          okHttpCertificate.getKeyStoreFile().toFile(), keyStorePassword);
  ...
  private TrustManagerFactory getTrustManagerFactory() {
    try {
      final KeyStore trustStore =
          KeyStore.getInstance(besuCertificate.getTrustStoreFile().toFile(),
                               besuCertificate.getPassword());
  ...
  private SSLContext
  getCustomSslContext(final TrustManagerFactory trustManagerFactory,
                      final Optional<KeyManagerFactory> keyManagerFactory)
      throws GeneralSecurityException {
    final KeyManager[] km =
        keyManagerFactory.map(KeyManagerFactory::getKeyManagers).orElse(null);
