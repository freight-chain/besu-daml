...
public class BlockchainQueriesTest {
  ...
  public void getBlockByNumberWithTxHashes() {
    ...
    final BlockWithMetadata<Hash, Hash> result =
        queries.blockByNumberWithTxHashes(targetBlock.getHeader().getNumber())
            .get();
    assertBlockMatchesResultWithTxHashes(targetBlock, result);
  }
  ...
  public void getBlockByNumberWithTxHashesForInvalidHash() {
    ...
    final Optional<BlockWithMetadata<Hash, Hash>> result =
        queries.blockByNumberWithTxHashes(10L);
    assertThat(result).isEmpty();
  }
  ...
  public void getLatestBlockWithTxHashes() {
    ...
    final BlockWithMetadata<Hash, Hash> result =
        queries.latestBlockWithTxHashes().get();
    assertBlockMatchesResultWithTxHashes(targetBlock, result);
  }
  ...
  public void getAccountStorageBlockNumber() {
    final List<Address> addresses =
        Arrays.asList(gen.address(), gen.address(), gen.address());
    ...
    final Hash latestStateRoot0 =
        data.blockData.get(2).block.getHeader().getStateRoot();
    final WorldState worldState0 =
        data.worldStateArchive.get(latestStateRoot0).get();
    addresses.forEach(address -> storageKeys.forEach(storageKey -> {
      final Account actualAccount0 = worldState0.get(address);
      final Optional<UInt256> result =
          queries.storageAt(address, storageKey, 2L);
      assertThat(result).contains(actualAccount0.getStorageValue(storageKey));
    }));

    final Hash latestStateRoot1 =
        data.blockData.get(1).block.getHeader().getStateRoot();
    final WorldState worldState1 =
        data.worldStateArchive.get(latestStateRoot1).get();
    addresses.forEach(address -> storageKeys.forEach(storageKey -> {
      final Account actualAccount1 = worldState1.get(address);
      final Optional<UInt256> result =
          queries.storageAt(address, storageKey, 1L);
  ...
  public void getAccountBalanceAtBlockNumber() {
    ...
    for (int i = 0; i < blockCount; i++) {
      ...
      final Hash stateRoot =
          data.blockData.get(i).block.getHeader().getStateRoot();
      ...
      addresses.forEach(address -> {
        ...
        final Optional<Wei> result =
            queries.accountBalance(address, curBlockNumber);
  ...
  public void getOmmerCountByHash() {
    ...
    final Optional<Integer> result =
        queries.getOmmerCount(targetBlock.getHash());
  ...
  public void getOmmerCountByNumber() {
    ...
    final Optional<Integer> result =
        queries.getOmmerCount(targetBlock.getHeader().getNumber());
    assertThat(result).contains(targetBlock.getBody().getOmmers().size());
  }
  ...
  public void getOmmerCountForLatestBlock() {
    ...
    final Block targetBlock =
        data.blockData.get(data.blockData.size() - 1).block;
  ...
  public void logsShouldBeFlaggedAsRemovedWhenBlockIsNotInCanonicalChain() {
    ...
    List<LogWithMetadata> logs = data.blockchainQueries.matchingLogs(
    ...
    final BlockOptions options =
        new BlockOptions()
            .setParentHash(
                data.blockchain.getBlockHashByNumber(commonAncestor).get())
            .setBlockNumber(forkBlock)
            .setDifficulty(data.blockchain.getBlockHeader(forkBlock)
                               .get()
                               .getDifficulty()
                               .add(10L));
    ...
    logs = data.blockchainQueries.matchingLogs(
        targetBlock.getHash(), new LogsQuery.Builder().build(), () -> true);
    assertThat(logs).isNotEmpty();
  ...
  public void matchingLogsShouldReturnAnEmptyListWhenGivenAnInvalidBlockHash() {
    final BlockchainWithData data = setupBlockchain(3);
    final BlockchainQueries queries = data.blockchainQueries;
    List<LogWithMetadata> logs = queries.matchingLogs(
        Hash.ZERO, new LogsQuery.Builder().build(), () -> true);
    assertThat(logs).isEmpty();
  }

  @Test
  public void
  getOmmerByBlockHashAndIndexShouldReturnEmptyWhenBlockDoesNotExist() {
    final BlockchainWithData data = setupBlockchain(3);
    final BlockchainQueries queries = data.blockchainQueries;
  ...
  @Test
  public void
  getOmmerByBlockHashAndIndexShouldReturnEmptyWhenBlockDoesNotHaveOmmers() {
    ...
    final Optional<BlockHeader> ommerOptional =
        queries.getOmmer(targetBlock.getHash(), 0);
  ...
  @Test
  public void
  getOmmerByBlockHashAndIndexShouldReturnEmptyWhenIndexIsOutOfRange() {
    final BlockchainWithData data = setupBlockchain(3);
    final BlockchainQueries queries = data.blockchainQueries;
    final Block targetBlock = data.blockData.get(0).block;
  ...
  public void getOmmerByBlockHashAndIndexShouldReturnExpectedOmmerHeader() {
    ...
    final Block targetBlock =
        data.blockData.get(data.blockData.size() - 1).block;
    final BlockHeader ommerBlockHeader =
        targetBlock.getBody().getOmmers().get(0);

    final BlockHeader retrievedOmmerBlockHeader =
        queries.getOmmer(targetBlock.getHash(), 0).get();
  ...
  @Test
  public void
  getOmmerByBlockNumberAndIndexShouldReturnEmptyWhenBlockDoesNotExist() {
    final BlockchainWithData data = setupBlockchain(3);
    final BlockchainQueries queries = data.blockchainQueries;
  ...
  @Test
  public void
  getOmmerByBlockNumberAndIndexShouldReturnEmptyWhenBlockDoesNotHaveOmmers() {
    final BlockchainWithData data = setupBlockchain(1);
    final BlockchainQueries queries = data.blockchainQueries;
    final Block targetBlock = data.blockData.get(0).block;
  ...
  @Test
  public void
  getOmmerByBlockNumberAndIndexShouldReturnEmptyWhenIndexIsOutOfRange() {
    final BlockchainWithData data = setupBlockchain(3);
    final BlockchainQueries queries = data.blockchainQueries;
    final Block targetBlock = data.blockData.get(0).block;
    final int indexOutOfRange = targetBlock.getBody().getOmmers().size() + 1;
  ...
  public void getOmmerByBlockNumberAndIndexShouldReturnExpectedOmmerHeader() {
    ...
    final Block targetBlock =
        data.blockData.get(data.blockData.size() - 1).block;
    final BlockHeader ommerBlockHeader =
        targetBlock.getBody().getOmmers().get(1);
  ...
  @Test
  public void
  getLatestBlockOmmerByIndexShouldReturnEmptyWhenLatestBlockDoesNotHaveOmmers() {
    final BlockchainWithData data = setupBlockchain(1);
    final BlockchainQueries queries = data.blockchainQueries;
    ...
    assertThat(
        queries.blockByNumber(queries.headBlockNumber()).get().getOmmers())
        .hasSize(0);
    assertThat(ommerOptional).isEmpty();
  }
  ...
  @Test
  public void
  getLatestBlockOmmerByIndexShouldReturnEmptyWhenIndexIsOutOfRange() {
    final BlockchainWithData data = setupBlockchain(3);
    final BlockchainQueries queries = data.blockchainQueries;
    final Block targetBlock =
        data.blockData.get(data.blockData.size() - 1).block;
    ...
    final Optional<BlockHeader> ommerOptional =
        queries.getOmmer(indexOutOfRange);
  ...
  private void assertBlockMatchesResult(
    ...
    final List<Hash> expectedOmmers = targetBlock.getBody()
                                          .getOmmers()
                                          .stream()
    ...
    for (int i = 0; i < result.getTransactions().size(); i++) {
      ...
      final Transaction targetTx =
          targetBlock.getBody().getTransactions().get(i);
      ...
      assertThat(txResult.getBlockNumber())
          .contains(targetBlock.getHeader().getNumber());
    }
  }
  ...
  private BlockchainWithData setupBlockchain(final int blocksToAdd) {
    return setupBlockchain(blocksToAdd, Collections.emptyList(),
                           Collections.emptyList());
  }

  private BlockchainWithData
  setupBlockchain(final int blocksToAdd, final List<Address> accountsToSetup) {
    return setupBlockchain(blocksToAdd, accountsToSetup,
                           Collections.emptyList());
  }
  ...
  setupBlockchain(final int blocksToAdd, final List<Address> accountsToSetup,
                  final List<UInt256> storageKeys) {
    checkArgument(blocksToAdd >= 1,
                  "Must add at least one block to the queries");
    ...
    final List<Block> blocks = gen.blockSequence(blocksToAdd, worldStateArchive,
                                                 accountsToSetup, storageKeys);
    ...
    final MutableBlockchain blockchain =
        createInMemoryBlockchain(blocks.get(0));
    blockData.subList(1, blockData.size())
  ...
  private static class BlockchainWithData {
    ...
    private BlockchainWithData(final MutableBlockchain blockchain,
                               final List<BlockData> blockData,
                               final WorldStateArchive worldStateArchive,
                               final EthScheduler scheduler) {
      ...
      this.blockchainQueries =
          new BlockchainQueries(blockchain, worldStateArchive, scheduler);
    }
  }
  ...
  private static class BlockData {
    ...
    private BlockData(final Block block,
                      final List<TransactionReceipt> receipts) {
      this.block = block;
