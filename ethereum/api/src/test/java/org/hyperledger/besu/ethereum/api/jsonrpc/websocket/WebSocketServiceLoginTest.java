...
public class WebSocketServiceLoginTest {
  ...
  public void before() throws URISyntaxException {
    ...
    final String authTomlPath =
        Paths
            .get(ClassLoader.getSystemResource("JsonRpcHttpService/auth.toml")
                     .toURI())
            .toAbsolutePath()
            .toString();
    ...
    final HttpClientRequest request = httpClient.post(
        ...
        "/login", response -> {
          ...
          response.bodyHandler(buffer -> {
            ...
            websocketService.authenticationService.get()
                .getJwtAuthProvider()
                .authenticate(new JsonObject().put("jwt", token), (r) -> {
                  ...
                  user.isAuthorized("eth:subscribe", (authed) -> {
                    assertThat(authed.succeeded()).isTrue();
                    assertThat(authed.result()).isTrue();
                    async.complete();
                  });
  ...
  @Test
  public void
  websocketServiceWithBadHeaderAuthenticationToken(final TestContext context) {
    final Async async = context.async();
    ...
    httpClient.websocket(options, headers, webSocket -> {
  ...
  @Test
  public void
  websocketServiceWithGoodHeaderAuthenticationToken(final TestContext context) {
    final Async async = context.async();
    ...
    final JsonObject jwtContents =
        new JsonObject()
            .put("permissions", Lists.newArrayList("eth:*"))
            .put("username", "user");
  ...
  @Test
  public void
  loginPopulatesJWTPayloadWithRequiredValues(final TestContext context) {
    final Async async = context.async();
    final HttpClientRequest request = httpClient.post(
        websocketConfiguration.getPort(), websocketConfiguration.getHost(),
        "/login", response -> {
          response.bodyHandler(buffer -> {
            ...
            assertThat(jwtPayload.getJsonArray("permissions"))
                .isEqualTo(
                    new JsonArray(list("fakePermission", "eth:blockNumber",
                                       "eth:subscribe", "web3:*")));
