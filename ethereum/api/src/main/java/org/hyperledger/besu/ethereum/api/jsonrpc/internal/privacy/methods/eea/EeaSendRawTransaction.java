...
public class EeaSendRawTransaction implements JsonRpcMethod {
  ...
  /*
   Temporarily adding this flag to this method to avoid being able to use
   offchain and onchain privacy groups at the same time. Later on this check
   will be done in a better place.
  */
  private final boolean onchainPrivacyGroupsEnabled;

  public EeaSendRawTransaction(final TransactionPool transactionPool,
  ...
  public JsonRpcResponse response(final JsonRpcRequestContext requestContext) {
    ...
    try {
      final PrivateTransaction privateTransaction = PrivateTransaction.readFrom(
          RLP.input(Bytes.fromHexString(rawPrivateTransaction)));

      final String enclavePublicKey =
          enclavePublicKeyProvider.getEnclaveKey(requestContext.getUser());

      if (!privateTransaction.getPrivateFrom().equals(
              Bytes.fromBase64String(enclavePublicKey))) {
        return new JsonRpcErrorResponse(
            id, PRIVATE_FROM_DOES_NOT_MATCH_ENCLAVE_PUBLIC_KEY);
      }
      ...
      if (onchainPrivacyGroupsEnabled) {
        if (!maybePrivacyGroupId.isPresent()) {
          return new JsonRpcErrorResponse(
              id, JsonRpcError.ONCHAIN_PRIVACY_GROUP_ID_NOT_AVAILABLE);
        }
        ...
        if (maybePrivacyGroup.isEmpty() &&
          ...
          return new JsonRpcErrorResponse(
              id, JsonRpcError.ONCHAIN_PRIVACY_GROUP_DOES_NOT_EXIST);
        }
      } else { // !onchainPirvacyGroupEnabled
        if (maybePrivacyGroupId.isPresent()) {
          maybePrivacyGroup = privacyController.retrieveOffChainPrivacyGroup(
      ...
      final ValidationResult<TransactionInvalidReason> validationResult =
          privacyController.validatePrivateTransaction(privateTransaction,
                                                       enclavePublicKey);
      if (!validationResult.isValid()) {
        return new JsonRpcErrorResponse(
            id, convertTransactionInvalidReason(
      ...
      if (onchainPrivacyGroupsEnabled) {
        final Bytes privacyGroupId = maybePrivacyGroupId.orElseThrow(
            ()
        ...
        final Optional<String> addPayloadEnclaveKey =
            privacyController.buildAndSendAddPayload(
                privateTransaction, Bytes32.wrap(privacyGroupId),
                enclavePublicKeyProvider.getEnclaveKey(
        ...
        return createPMTAndAddToTxPool(id, privateTransaction, enclaveKey,
  ...
  JsonRpcResponse createPMTAndAddToTxPool(
      final Object id, final PrivateTransaction privateTransaction,
      final String payload, final Address privacyPrecompileAddress) {
    final Transaction privacyMarkerTransaction;
    privacyMarkerTransaction = privacyController.createPrivacyMarkerTransaction(
        payload, privateTransaction, privacyPrecompileAddress);
    return transactionPool.addLocalTransaction(privacyMarkerTransaction)
        .either(()
                    -> new JsonRpcSuccessResponse(
                        id, privacyMarkerTransaction.getHash().toString()),
                errorReason -> getJsonRpcErrorResponse(id, errorReason));
  }

  JsonRpcErrorResponse
  getJsonRpcErrorResponse(final Object id,
                          final TransactionInvalidReason errorReason) {
    if (errorReason.equals(
            TransactionInvalidReason.INTRINSIC_GAS_EXCEEDS_GAS_LIMIT)) {
      return new JsonRpcErrorResponse(
          id, JsonRpcError.PMT_FAILED_INTRINSIC_GAS_EXCEEDS_LIMIT);
    }
    return new JsonRpcErrorResponse(
        id, convertTransactionInvalidReason(errorReason));
  }

  private String buildCompoundKey(final String enclaveKey,
                                  final Optional<String> addPayloadEnclaveKey) {
    return addPayloadEnclaveKey.isPresent()
        ? Bytes
              .concatenate(Bytes.fromBase64String(enclaveKey),
