...
public class AuthenticationUtils {
  ...
  isPermitted(final Optional<AuthenticationService> authenticationService,
    ...
    if (optionalUser.isPresent()) {
      ...
      for (String perm : jsonRpcMethod.getPermissions()) {
        user.isAuthorized(perm, (authed) -> {
          if (authed.result()) {
            LOG.trace("user {} authorized : {} via permission {}", user,
                      jsonRpcMethod.getName(), perm);
  ...
  public static void
  getUser(final Optional<AuthenticationService> authenticationService,
          final String token, final Handler<Optional<User>> handler) {
    try {
      if (authenticationService.isEmpty()) {
        handler.handle(Optional.empty());
      } else {
        authenticationService.get().getJwtAuthProvider().authenticate(
            new JsonObject().put("jwt", token), (r) -> {
              if (r.succeeded()) {
  ...
  private static void validateExpiryExists(final Optional<User> user) {
    if (!user.map(User::principal)
             .map(p -> p.containsKey("exp"))
             .orElse(false)) {
  ...
  getJwtTokenFromAuthorizationHeaderValue(final String value) {
    if (value != null) {
