...
public class JsonRpcHttpService {
  ...
  public CompletableFuture<?> start() {
    LOG.info("Starting JSON-RPC service on {}:{}", config.getHost(),
             config.getPort());
    ...
    try {
      ...
      httpServer.requestHandler(buildRouter()).listen(res -> {
        if (!res.failed()) {
          ...
          LOG.info("JSON-RPC service started and listening on {}:{}{}",
    ...
    vertx.executeBlocking(
        ...
        -> {
          final JsonRpcResponse jsonRpcResponse =
              process(routingContext, request, user);
    ...
    final List<Future> responses =
        jsonArray.stream()
            .map(obj -> {
              ...
              vertx.executeBlocking(
                  future
                  -> future.complete(process(routingContext, req, user)),
                  false, ar -> {
                    if (ar.failed()) {
                      fut.fail(ar.cause());
                    } else {
  ...
  private JsonRpcResponse process(final RoutingContext ctx,
      ...
      try (final OperationTimer.TimingContext ignored =
        ...
        if (user.isPresent()) {
          return method.response(new JsonRpcRequestContext(
              requestBody, user.get(), () -> !ctx.response().closed()));
        }
        return method.response(new JsonRpcRequestContext(
            requestBody, () -> !ctx.response().closed()));
