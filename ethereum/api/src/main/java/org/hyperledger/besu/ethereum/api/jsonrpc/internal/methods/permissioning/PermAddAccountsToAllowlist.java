    ...
    if (allowlistController.isPresent()) {
      ...
      case ERROR_EMPTY_ENTRY:
        return new JsonRpcErrorResponse(
            ...
            JsonRpcError.ACCOUNT_ALLOWLIST_EMPTY_ENTRY);
      case ERROR_INVALID_ENTRY:
        return new JsonRpcErrorResponse(
            requestContext.getRequest().getId(),
            JsonRpcError.ACCOUNT_ALLOWLIST_INVALID_ENTRY);
      case ERROR_EXISTING_ENTRY:
        return new JsonRpcErrorResponse(
            requestContext.getRequest().getId(),
            JsonRpcError.ACCOUNT_ALLOWLIST_EXISTING_ENTRY);
      case ERROR_DUPLICATED_ENTRY:
        return new JsonRpcErrorResponse(
            requestContext.getRequest().getId(),
            JsonRpcError.ACCOUNT_ALLOWLIST_DUPLICATED_ENTRY);
