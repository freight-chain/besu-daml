...
public class PrivGetPrivateTransaction implements JsonRpcMethod {
  ...
  public PrivGetPrivateTransaction(final PrivacyController privacyController,
  ...
  public JsonRpcResponse response(final JsonRpcRequestContext requestContext) {
    LOG.trace("Executing {}",
              RpcMethod.PRIV_GET_PRIVATE_TRANSACTION.getMethodName());
    ...
    try {
      maybePrivateTx =
          privacyController.findPrivateTransactionByPmtHash(hash, enclaveKey)
    ...
    return maybePrivateTx.map(this::mapTransactionResult)
        .map(result
             -> new JsonRpcSuccessResponse(requestContext.getRequest().getId(),
                                           result))
        .orElse(new JsonRpcSuccessResponse(requestContext.getRequest().getId(),
                                           null));
  }

  private PrivateTransactionResult
  mapTransactionResult(final PrivateTransaction privateTransaction) {
