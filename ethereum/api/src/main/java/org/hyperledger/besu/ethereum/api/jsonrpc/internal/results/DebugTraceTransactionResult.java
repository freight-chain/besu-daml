...
public class DebugTraceTransactionResult {
  ...
  public DebugTraceTransactionResult(final TransactionTrace transactionTrace) {
    ...
    returnValue =
        transactionTrace.getResult().getOutput().toString().substring(2);
    structLogs = transactionTrace.getTraceFrames()
                     .stream()
  ...
  public static Collection<DebugTraceTransactionResult>
  of(final Collection<TransactionTrace> traces) {
  ...
  private static StructLog createStructLog(final TraceFrame frame) {
    return frame.getExceptionalHaltReason()
