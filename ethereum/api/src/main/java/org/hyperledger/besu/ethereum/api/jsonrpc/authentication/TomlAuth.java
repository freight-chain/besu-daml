...
public class TomlAuth implements AuthProvider {
  ...
  @Override
  public void authenticate(final JsonObject authInfo,
    ...
    vertx.executeBlocking(
        ...
        -> {
          ...
          final TomlTable userData =
              parseResult.getTableOrEmpty("Users." + username);
          if (userData.isEmpty()) {
        ...
        res -> {
          if (res.succeeded()) {
            resultHandler.handle(
                Future.succeededFuture((TomlUser)res.result()));
          } else {
  ...
  private TomlUser readTomlUserFromTable(final String username,
                                         final TomlTable userData) {
    final String saltedAndHashedPassword =
        userData.getString("password", () -> "");
    final List<String> groups = userData.getArrayOrEmpty("groups")
                                    .toList()
                                    .stream()
                                    .map(Object::toString)
                                    .collect(Collectors.toList());
    final List<String> permissions = userData.getArrayOrEmpty("permissions")
                                         .toList()
                                         .stream()
                                         .map(Object::toString)
                                         .collect(Collectors.toList());
    final List<String> roles = userData.getArrayOrEmpty("roles")
                                   .toList()
                                   .stream()
                                   .map(Object::toString)
                                   .collect(Collectors.toList());
    ...
    return new TomlUser(username, saltedAndHashedPassword, groups, permissions,
                        roles, privacyPublicKey);
  }

  private void
  checkPasswordHash(final String password, final String passwordHash,
