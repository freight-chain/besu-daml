...
public class BackendQuery {
  ...
  public static <T> T runIfAlive(final Callable<T> task,
                                 final Supplier<Boolean> alive)
      throws Exception {
    return runIfAlive(Optional.empty(), task, alive);
  }

  public static <T> T runIfAlive(final String taskName, final Callable<T> task,
                                 final Supplier<Boolean> alive)
      throws Exception {
    return runIfAlive(Optional.ofNullable(taskName), task, alive);
  }

  public static <T> T runIfAlive(final Optional<String> taskName,
                                 final Callable<T> task,
                                 final Supplier<Boolean> alive)
      throws Exception {
    if (!alive.get()) {
      LOG.warn("Zombie backend query detected [ {} ], aborting process.",
               taskName.orElse("unnamed"));
      throw new RpcMethodTimeoutException();
    }
    return task.call();
  }

  public static void stopIfExpired(final Supplier<Boolean> alive)
      throws Exception {
