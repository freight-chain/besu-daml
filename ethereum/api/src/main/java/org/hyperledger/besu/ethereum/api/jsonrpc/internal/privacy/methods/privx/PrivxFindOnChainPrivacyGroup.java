...
public class PrivxFindOnChainPrivacyGroup implements JsonRpcMethod {
  ...
  public PrivxFindOnChainPrivacyGroup(final PrivacyController privacyController,
  ...
  public JsonRpcResponse response(final JsonRpcRequestContext requestContext) {
    LOG.trace("Executing {}",
              RpcMethod.PRIVX_FIND_PRIVACY_GROUP.getMethodName());

    final String[] addresses =
        requestContext.getRequiredParameter(0, String[].class);

    LOG.trace("Finding a privacy group with members {}",
              Arrays.toString(addresses));

    final List<PrivacyGroup> response;
    try {
      response = privacyController.findOnChainPrivacyGroup(
      ...
      return new JsonRpcErrorResponse(requestContext.getRequest().getId(),
                                      FIND_ONCHAIN_PRIVACY_GROUP_ERROR);
