...
public class PermissioningConfigurationBuilder {
  ...
  public static SmartContractPermissioningConfiguration
  smartContractPermissioningConfiguration(
      final Address address,
      final boolean smartContractPermissionedNodeEnabled) {
    ...
    config.setSmartContractNodeAllowlistEnabled(
        smartContractPermissionedNodeEnabled);
    return config;
  }

  public static LocalPermissioningConfiguration
  permissioningConfiguration(final boolean nodePermissioningEnabled,
    ...
    loadNodePermissioning(permissioningConfiguration, nodePermissioningEnabled,
                          nodePermissioningConfigFilepath);
    loadAccountPermissioning(permissioningConfiguration,
  ...
  private static LocalPermissioningConfiguration loadNodePermissioning(
      ...
      final String nodePermissioningConfigFilepath) throws Exception {

    if (localConfigNodePermissioningEnabled) {
      final TomlParseResult nodePermissioningToml =
          readToml(nodePermissioningConfigFilepath);
      final TomlArray nodeAllowlistTomlArray = getAllowlistArray(
          nodePermissioningToml, NODES_ALLOWLIST_KEY, NODES_WHITELIST_KEY);

      permissioningConfiguration.setNodePermissioningConfigFilePath(
          nodePermissioningConfigFilepath);

      if (nodeAllowlistTomlArray != null) {
        List<URI> nodesAllowlistToml = nodeAllowlistTomlArray.toList()
                                           .parallelStream()
                                           .map(Object::toString)
                                           .map(EnodeURL::asURI)
        ...
        throw new Exception(NODES_ALLOWLIST_KEY +
  ...
  private static LocalPermissioningConfiguration loadAccountPermissioning(
      ...
      final String accountPermissioningConfigFilepath) throws Exception {

    if (localConfigAccountPermissioningEnabled) {
      final TomlParseResult accountPermissioningToml =
          readToml(accountPermissioningConfigFilepath);
      ...
      if (accountAllowlistTomlArray != null) {
        List<String> accountsAllowlistToml = accountAllowlistTomlArray.toList()
                                                 .parallelStream()
                                                 .map(Object::toString)
                                                 .collect(Collectors.toList());
        ...
        throw new Exception(ACCOUNTS_ALLOWLIST_KEY +
                            " config option missing in TOML config file " +
                            accountPermissioningConfigFilepath);
      }
    }
  ...
  /**
   * This method allows support for both keys for now. Whitelist TOML keys will
   * be removed in future (breaking change)
   *
   * @param tomlParseResult result of a prior toml parse
   * @param primaryKey key to fetch
   * @param alternateKey alternate key to fetch
   * @return In order: the array of the primaryKey if it exists, or the array of
   *     the alternateKey if it exists, or null.
   */
  private static TomlArray
  getAllowlistArray(final TomlParseResult tomlParseResult,
                    final String primaryKey, final String alternateKey) {
  ...
  private static TomlParseResult readToml(final String filepath)
      throws Exception {
    ...
    try {
      ...
      throw new Exception("Unable to read permissioning TOML config file : " +
