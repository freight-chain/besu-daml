...
/**
 * TCP server allowing miners to connect to the client over persistent TCP
 * connections, using the various Stratum protocols.
 */
public class StratumServer implements EthHashObserver {
  ...
  public StratumServer(final Vertx vertx,
                       final MiningCoordinator miningCoordinator,
                       final int port, final String networkInterface,
                       final String extraNonce) {
    this.vertx = vertx;
    this.port = port;
    this.networkInterface = networkInterface;
    protocols = new StratumProtocol[] {
  ...
  public CompletableFuture<?> start() {
    if (started.compareAndSet(false, true)) {
      ...
      server.listen(res -> {
        if (res.failed()) {
          result.completeExceptionally(new StratumServerException(String.format(
  ...
  private void handle(final NetSocket socket) {
    StratumConnection conn = new StratumConnection(
  ...
  public void newJob(final EthHashSolverInputs ethHashSolverInputs) {
    if (!started.get()) {
      logger.debug("Discarding {} as stratum server is not started",
                   ethHashSolverInputs);
      return;
    }
