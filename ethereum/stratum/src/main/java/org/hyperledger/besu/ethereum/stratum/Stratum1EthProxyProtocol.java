...
public class Stratum1EthProxyProtocol implements StratumProtocol {
  ...
  public boolean canHandle(final String initialMessage,
    ...
    if (!"eth_submitLogin".equals(req.getRequest().getMethod())) {
      LOG.debug("Invalid first message method: {}",
    ...
    try {
      String response = mapper.writeValueAsString(
          new JsonRpcSuccessResponse(req.getRequest().getId(), true));
      conn.send(response + "\n");
  ...
  public void handle(final StratumConnection conn, final String message) {
    try {
      final JsonRpcRequest req =
          new JsonObject(message).mapTo(JsonRpcRequest.class);
      if (RpcMethod.ETH_GET_WORK.getMethodName().equals(req.getMethod())) {
        sendNewWork(conn, req.getId());
      } else if (RpcMethod.ETH_SUBMIT_WORK.getMethodName().equals(
                     req.getMethod())) {
  ...
  private void handleMiningSubmit(final StratumConnection conn,
                                  final JsonRpcRequest req) throws IOException {
    LOG.debug("Miner submitted solution {}", req);
    boolean result = false;
    final EthHashSolution solution = new EthHashSolution(
        Bytes.fromHexString(req.getRequiredParameter(0, String.class))
            .getLong(0),
    ...
    String response = mapper.writeValueAsString(
        new JsonRpcSuccessResponse(req.getId(), result));
    conn.send(response + "\n");
  }
