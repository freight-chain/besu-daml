 ...
 *
 * <p>Manages the lifecycle of a TCP connection according to a particular
 * variant of the Stratum protocol.
 */
public interface StratumProtocol {

  /**
   * Checks if the protocol can handle a TCP connection, based on the initial
   * message.
   *
  ...
  void
  setSubmitCallback(Function<EthHashSolution, Boolean> submitSolutionCallback);

  default void handleHashrateSubmit(final JsonMapper mapper,
    ...
    String response = mapper.writeValueAsString(new JsonRpcSuccessResponse(
        message.getId(),
