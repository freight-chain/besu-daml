...
public class StratumConnectionTest {
  ...
  public void testNoSuitableProtocol() {
    ...
    StratumConnection conn = new StratumConnection(
        new StratumProtocol[] {}, () -> called.set(true), bytes -> {});
  ...
  public void testStratum1Matches() {
    ...
    StratumConnection conn = new StratumConnection(
        new StratumProtocol[] {new Stratum1Protocol(
            "", miningCoordinator, () -> "abcd", () -> "abcd")},
        () -> called.set(true), message::set);
    conn.handleBuffer(
        Buffer.buffer("{"
                      + "  \"id\": 23,"
  ...
  public void testStratum1SendWork() {
    ...
    protocol.setCurrentWorkTask(new EthHashSolverInputs(
        UInt256.valueOf(3), Bytes.fromHexString("deadbeef").toArrayUnsafe(),
        42));
