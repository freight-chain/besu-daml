...
public class MainnetTransactionValidatorTest {
  ...
  public void shouldRejectTransactionIfIntrinsicGasExceedsGasLimit() {
    ...
    final Transaction transaction = new TransactionTestFixture()
                                        ...
                                        .createTransaction(senderKeys);
    when(gasCalculator.transactionIntrinsicGasCost(transaction))
        .thenReturn(Gas.of(50));
  ...
  @Test
  public void
  shouldRejectTransactionWhenTransactionHasChainIdAndValidatorDoesNot() {
    final MainnetTransactionValidator validator =
        new MainnetTransactionValidator(gasCalculator, false, Optional.empty());
  ...
  public void transactionWithNullSenderCanBeValidIfGasPriceAndValueIsZero() {
    ...
    assertThat(validator.validateForSender(
                   builder.createTransaction(senderKeyPair), null, false))
        .isEqualTo(ValidationResult.valid());
  }
  ...
  public void shouldRejectTransactionIfAccountIsNotPermitted() {
    ...
    assertThat(validator.validateForSender(basicTransaction,
                                           accountWithNonce(0), true))
        .isEqualTo(ValidationResult.invalid(
            TransactionValidator.TransactionInvalidReason
  ...
  public void shouldPropagateCorrectStateChangeParamToTransactionFilter() {
    final ArgumentCaptor<Boolean> stateChangeParamCaptor =
        ArgumentCaptor.forClass(Boolean.class);
    final TransactionFilter transactionFilter = mock(TransactionFilter.class);
    when(transactionFilter.permitted(any(Transaction.class),
                                     stateChangeParamCaptor.capture()))
        .thenReturn(true);
    ...
    validator.validateForSender(basicTransaction, accountWithNonce(0),
                                validationParams);
  ...
  @Test
  public void
  shouldNotCheckAccountPermissionIfBothValidationParamsCheckPermissionsAreFalse() {
    final TransactionFilter transactionFilter = mock(TransactionFilter.class);
    ...
    assertThat(validator.validateForSender(
                   basicTransaction, accountWithNonce(0), validationParams))
        .isEqualTo(ValidationResult.valid());
  ...
  public void shouldRejectTransactionIfLegacyAfterEIP1559Finalized() {
    ...
    final MainnetTransactionValidator validator =
        new MainnetTransactionValidator(
            gasCalculator, false, Optional.empty(), Optional.of(eip1559),
  ...
  private TransactionFilter transactionFilter(final boolean permitted) {
    final TransactionFilter transactionFilter = mock(TransactionFilter.class);
    when(transactionFilter.permitted(any(Transaction.class), anyBoolean()))
