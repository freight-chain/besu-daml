...
public class SStoreOperationTest {
  ...
  private static final GasCalculator gasCalculator =
      new ConstantinopleGasCalculator();
  ...
  public SStoreOperationTest(final Gas minimumGasAvailable,
                             final Gas initialGas, final Gas remainingGas,
  ...
  @Parameterized.Parameters(
      name =
          "{index}: minimum gas {0}, initial gas {1}, remaining gas {2}, expected halt {3}")
  ...
  private MessageFrame createMessageFrame(final Address address,
    ...
    final WorldUpdater worldStateUpdater =
        worldStateArchive.getMutable().updater();
    final BlockHeader blockHeader = new BlockHeaderTestFixture().buildHeader();
    final MessageFrame frame = new MessageFrameTestFixture()
                                   .address(address)
  ...
  public void storeOperation() {
    final SStoreOperation operation =
        new SStoreOperation(gasCalculator, minimumGasAvailable);
    final MessageFrame frame = createMessageFrame(
        Address.fromHexString("0x18675309"), initialGas, remainingGas);
    assertThat(operation.exceptionalHaltCondition(frame, null))
        .isEqualTo(expectedHalt);
  }
}
