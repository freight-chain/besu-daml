...
public class PrivateTransactionDataFixture {
  ...
  public static final Bytes32 VALID_BASE64_ENCLAVE_KEY = Bytes32.wrap(
  ...
  public static Transaction privacyMarkerTransaction() {
    return privacyMarkerTransaction(VALID_BASE64_ENCLAVE_KEY,
  ...
  privacyMarkerTransaction(final Bytes transactionKey,
                           final Address precompiledContractAddress) {
  ...
  public static PrivateTransaction
  ...
  public static ReceiveResponse
  generateReceiveResponse(final PrivateTransaction privateTransaction) {
  ...
  public static ReceiveResponse
  generateAddToGroupReceiveResponse(final PrivateTransaction privateTransaction,
    ...
    final BytesValueRLPOutput rlpOutput = new BytesValueRLPOutput();
    rlpOutput.startList();
    privateTransactionWithMetadataList.stream().forEach(
  ...
  public static List<PrivateTransactionWithMetadata>
  generateAddBlobResponse(final PrivateTransaction privateTransaction,
                          final Transaction markerTransaction) {
    final PrivateTransactionWithMetadata privateTransactionWithMetadata =
        new PrivateTransactionWithMetadata(
            privateTransaction, new PrivateTransactionMetadata(
                                    markerTransaction.getHash(), Hash.ZERO));
    return Collections.singletonList(privateTransactionWithMetadata);
  }

  public static PrivateTransactionMetadata
  generatePrivateTransactionMetadata() {
    return new PrivateTransactionMetadata(Hash.hash(Bytes32.random()),
                                          Hash.hash(Bytes32.random()));
  }

  public static List<PrivateTransactionMetadata>
  generatePrivateTransactionMetadataList(final int length) {
    return IntStream.range(0, length)
        .mapToObj((i) -> generatePrivateTransactionMetadata())
        .collect(Collectors.toList());
  }

  public static PrivateBlockMetadata
  generatePrivateBlockMetadata(final int numberOfTransactions) {
    return new PrivateBlockMetadata(
        generatePrivateTransactionMetadataList(numberOfTransactions));
  }

  public static Bytes
  encodePrivateTransaction(final PrivateTransaction privateTransaction,
