...
public interface Blockchain {
  /**
   * Return the head (latest block hash and total difficulty) of the local
  ...
  default BlockHeader getChainHeadHeader() {
    return getBlockHeader(getChainHeadHash())
        .orElseThrow(
            () -> new IllegalStateException("Missing chain head header."));
  }
  ...
  default Block getChainHeadBlock() {
    ...
    final BlockBody body =
        getBlockBody(chainHeadHash)
            .orElseThrow(
                () -> new IllegalStateException("Missing chain head body."));
  ...
  default Block getGenesisBlock() {
    final Hash genesisHash =
        getBlockHashByNumber(BlockHeader.GENESIS_BLOCK_NUMBER)
            .orElseThrow(
                () -> new IllegalStateException("Missing genesis block."));
  ...
  default Optional<Block> getBlockByHash(final Hash blockHash) {
    return getBlockHeader(blockHash).flatMap(
        header -> getBlockBody(blockHash).map(body -> new Block(header, body)));
  }
  ...
  /**
   * Checks whether the block corresponding to the given hash is on the
   * canonical chain.
   *
   * @param blockHeaderHash The hash of the block to check.
   * @return true if the block corresponding to the hash is on the canonical
   *     chain.
   */
  ...
  /**
   * Returns the block header corresponding to the given block number on the
   * canonical chain.
   *
   * @param blockNumber The reference block number whose header we want to
   *     retrieve.
  ...
  /**
   * Returns the block header corresponding to the given block hash. Associated
   * block is not necessarily on the canonical chain.
   *
   * @param blockHeaderHash The hash of the block whose header we want to
   *     retrieve.
  ...
  /**
   * Returns the block body corresponding to the given block header hash.
   * Associated block is not necessarily on the canonical chain.
   *
   * @param blockHeaderHash The block header hash identifying the block whose
  ...
  /**
   * Given a block's hash, returns the list of transaction receipts associated
   * with this block's transactions. Associated block is not necessarily on the
   * canonical chain.
   *
  ...
  /**
   * Returns the total difficulty (cumulative difficulty up to and including the
   * target block) of the block corresponding to the given hash. Associated
   * block is not necessarily on the canonical chain.
   *
  ...
  /**
   * Given a transaction hash, returns the location (block number and
   * transaction index) of the hashed transaction on the canonical chain.
   *
  ...
  /**
   * Adds an observer that will get called on for every added and removed log
   * when a new block is added.
   *
   * <p><i>No guarantees are made about the order in which the observers are
   * invoked.</i>
   *
  ...
  default long observeLogs(final Consumer<LogWithMetadata> logObserver) {
    return observeBlockAdded(
        (event -> event.getLogsWithMetadata().forEach(logObserver)));
  }
  ...
  /**
   * Adds an observer that will get called when a new block is added after
   * reorg.
   *
   * <p><i>No guarantees are made about the order in which observers are
   * invoked.</i>
