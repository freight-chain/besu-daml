...
public class EIP1559BlockHeaderGasPriceValidationRule
    implements DetachedBlockHeaderValidationRule {
  private final Logger LOG =
      LogManager.getLogger(EIP1559BlockHeaderGasPriceValidationRule.class);
      ...
      if (eip1559.isForkBlock(header.getNumber())) {
        return feeMarket.getInitialBasefee() ==
            header.getBaseFee().orElseThrow(
                EIP1559MissingBaseFeeFromBlockHeader::new);
      }

      final Long parentBaseFee = parent.getBaseFee().orElseThrow(
          EIP1559MissingBaseFeeFromBlockHeader::new);
      final Long currentBaseFee = header.getBaseFee().orElseThrow(
          EIP1559MissingBaseFeeFromBlockHeader::new);
      final long baseFee =
          eip1559.computeBaseFee(parentBaseFee, parent.getGasUsed());
      if (baseFee != header.getBaseFee().orElseThrow()) {
        LOG.trace(
            "Invalid block header: basefee {} does not equal expected basefee {}",
            header.getBaseFee().orElseThrow(), baseFee);
        return false;
      }
      return baseFee == currentBaseFee &&
          eip1559.isValidBaseFee(parentBaseFee, currentBaseFee);
