...
public interface EthHasher {
  ...
  final class Light implements EthHasher {
    ...
    private static final EthHashCacheFactory cacheFactory =
        new EthHashCacheFactory();

    @Override
    public void hash(final byte[] buffer, final long nonce, final long number,
                     final byte[] headerHash) {
      final EthHashCacheFactory.EthHashDescriptor cache =
          cacheFactory.ethHashCacheFor(number);
      final byte[] hash = EthHash.hashimotoLight(
          cache.getDatasetSize(), cache.getCache(), headerHash, nonce);
