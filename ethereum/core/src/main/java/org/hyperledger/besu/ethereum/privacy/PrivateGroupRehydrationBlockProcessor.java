...
public class PrivateGroupRehydrationBlockProcessor {
    ...
    for (final Transaction transaction : transactions) {
      ...
      if (forExecution.containsKey(transaction.getHash())) {
        final PrivateTransaction privateTransaction =
            forExecution.get(transaction.getHash());
        ...
        final WorldUpdater privateStateUpdater =
        ...
        LOG.debug("Pre-rehydrate root hash: {} for tx {}",
                  disposablePrivateState.rootHash(), transaction.getHash());

        final PrivateTransactionProcessor.Result privateResult =
            privateTransactionProcessor.processTransaction(
                blockchain, worldStateUpdater.updater(), privateStateUpdater,
                blockHeader, transaction.getHash(), privateTransaction,
                miningBeneficiary, OperationTracer.NO_TRACING,
                new BlockHashLookup(blockHeader, blockchain),
                privateTransaction.getPrivacyGroupId().get());
        persistPrivateState(
            transaction.getHash(), blockHeader.getHash(), privateTransaction,
            Bytes32.wrap(privateTransaction.getPrivacyGroupId().get()),
            disposablePrivateState, privateStateUpdater, privateStateStorage,
            privateResult);
        LOG.debug("Post-rehydrate root hash: {}",
  ...
  protected void
  persistPrivateState(final Hash commitmentHash, final Hash currentBlockHash,
                      final PrivateTransaction privateTransaction,
    ...
    updatePrivateBlockMetadata(commitmentHash, currentBlockHash, privacyGroupId,
                               disposablePrivateState.rootHash(),
                               privateStateUpdater, privateStateStorage);
