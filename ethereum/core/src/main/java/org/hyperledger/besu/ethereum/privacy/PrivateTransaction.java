...
public class PrivateTransaction {
  ...
  private static final BigInteger REPLAY_UNPROTECTED_V_BASE =
      BigInteger.valueOf(27);
  private static final BigInteger REPLAY_UNPROTECTED_V_BASE_PLUS_1 =
      BigInteger.valueOf(28);

  private static final BigInteger REPLAY_PROTECTED_V_BASE =
      BigInteger.valueOf(35);

  // The v signature parameter starts at 36 because 1 is the first valid chainId
  // so: chainId > 1 implies that 2 * chainId + V_BASE > 36.
  private static final BigInteger REPLAY_PROTECTED_V_MIN =
  ...
  // not fully identify the transaction (use the result of the {@code hash()}
  // for that). It is only used to compute said signature and recover the sender
  // from it.
  ...
  @Deprecated(since = "1.4.3") protected volatile Hash hash;

  public static Builder builder() { return new Builder(); }

  @SuppressWarnings({"unchecked"})
  public static PrivateTransaction readFrom(final RLPInput input)
      throws RLPException {
    input.enterList();
    ...
    if (v.equals(REPLAY_UNPROTECTED_V_BASE) ||
        v.equals(REPLAY_UNPROTECTED_V_BASE_PLUS_1)) {
      recId = v.subtract(REPLAY_UNPROTECTED_V_BASE).byteValueExact();
    } else if (v.compareTo(REPLAY_PROTECTED_V_MIN) > 0) {
      chainId = Optional.of(v.subtract(REPLAY_PROTECTED_V_BASE).divide(TWO));
      recId =
          v.subtract(TWO.multiply(chainId.get()).add(REPLAY_PROTECTED_V_BASE))
              .byteValueExact();
    } else {
    ...
    final SECP256K1.Signature signature =
        SECP256K1.Signature.create(r, s, recId);

    final Bytes privateFrom = input.readBytes();
    final Object privateForOrPrivacyGroupId =
        resolvePrivateForOrPrivacyGroupId(input.readAsRlp());
  ...
  private static Object resolvePrivateForOrPrivacyGroupId(final RLPInput item) {
    return item.nextIsList() ? item.readList(RLPInput::readBytes)
                             : item.readBytes();
  }
   ...
   *     <p>The {@code to} will be an {@code Optional.empty()} for a contract
   * creation transaction; otherwise it should contain an address. <p>The {@code
   * chainId} must be greater than 0 to be applied to a specific chain;
   * otherwise it will default to any chain.
   ...
   * @param privateFor An array of the public keys of the intended recipients of
   *     this private transaction
  ...
  protected PrivateTransaction(
      ...
      final SECP256K1.Signature signature, final Bytes payload,
      final Address sender, final Optional<BigInteger> chainId,
      final Bytes privateFrom, final Optional<List<Bytes>> privateFor,
      final Optional<Bytes> privacyGroupId, final Restriction restriction) {
    this.nonce = nonce;
  ...
  protected PrivateTransaction(final PrivateTransaction privateTransaction) {
    this(privateTransaction.getNonce(), privateTransaction.getGasPrice(),
         privateTransaction.getGasLimit(), privateTransaction.getTo(),
         privateTransaction.getValue(), privateTransaction.getSignature(),
         privateTransaction.getPayload(), privateTransaction.getSender(),
         privateTransaction.getChainId(), privateTransaction.getPrivateFrom(),
   ...
   *
   * <p>The {@code Optional<Address>} will be {@code Optional.empty()} if the
   * transaction is a contract creation; otherwise it will contain the message
   * call transaction recipient.
   *
   * @return the transaction recipient if a message call; otherwise {@code
   *     Optional.empty()}
   */
  public Optional<Address> getTo() { return to; }
   ...
   */
  public SECP256K1.Signature getSignature() { return signature; }
   ...
   *
   * <p>The {@code OptionalInt} will be {@code OptionalInt.empty()} if the
   * transaction is not tied to a specific chain.
   *
   * @return the transaction chain id if it exists; otherwise {@code
   *     OptionalInt.empty()}
   */
  public Optional<BigInteger> getChainId() { return chainId; }
   ...
   */
  public Bytes getPrivateFrom() { return privateFrom; }
   ...
   */
  public Optional<List<Bytes>> getPrivateFor() { return privateFor; }
   ...
   */
  public Optional<Bytes> getPrivacyGroupId() { return privacyGroupId; }
  ...
  public Address getSender() {
    if (sender == null) {
      final SECP256K1.PublicKey publicKey =
          SECP256K1.PublicKey
              .recoverFromSignature(getOrComputeSenderRecoveryHash(), signature)
              .orElseThrow(()
                               -> new IllegalStateException(
                                   "Cannot recover public key from "
                                   + "signature for " + this));
  ...
  private Bytes32 getOrComputeSenderRecoveryHash() {
    if (hashNoSignature == null) {
      hashNoSignature = computeSenderRecoveryHash(
          ...
          privateFrom, privateFor, privacyGroupId, restriction.getBytes());
    }
  ...
  public void writeTo(final RLPOutput out) {
    ...
    getPrivateFor().ifPresent(
        privateFor
        -> out.writeList(privateFor, (bv, rlpO) -> rlpO.writeBytes(bv)));
  ...
  public BigInteger getR() { return signature.getR(); }

  public BigInteger getS() { return signature.getS(); }
   ...
   *
   * @deprecated All private transactions should be identified by their
   *     corresponding PMT hash.
   ...
   *     {@code false}
   */
  public boolean isContractCreation() { return !getTo().isPresent(); }
   ...
   * is executed. The sender must have the amount in its account balance to
   * execute and some of this amount may be refunded after the transaction has
   * executed.
   *
   * @return the up-front gas cost for the transaction
   */
  public Wei getUpfrontCost() { return getUpfrontGasCost().add(getValue()); }

  /**
   * Determines the privacy group id. Either returning the value of
   * privacyGroupId field if it exists or calculating the EEA privacyGroupId
   * from the privateFrom and privateFor fields.
   *
  ...
  public Bytes32 determinePrivacyGroupId() {
    if (getPrivacyGroupId().isPresent()) {
      ...
      final List<Bytes> privateFor =
          getPrivateFor().orElse(Lists.newArrayList());
      return PrivacyGroupUtil.calculateEeaPrivacyGroupId(getPrivateFrom(),
                                                         privateFor);
    }
  }
  ...
  private static Bytes32 computeSenderRecoveryHash(
      final long nonce, final Wei gasPrice, final long gasLimit,
      final Address to, final Wei value, final Bytes payload,
      final Optional<BigInteger> chainId, final Bytes privateFrom,
      final Optional<List<Bytes>> privateFor,
      final Optional<Bytes> privacyGroupId, final Bytes restriction) {
    return keccak256(RLP.encode(out -> {
      out.startList();
      ...
      privateFor.ifPresent(
          pF -> out.writeList(pF, (bv, rlpO) -> rlpO.writeBytes(bv)));
  ...
  public boolean equals(final Object other) {
    ...
    return this.chainId.equals(that.chainId) &&
        this.gasLimit == that.gasLimit && this.gasPrice.equals(that.gasPrice) &&
        this.nonce == that.nonce && this.payload.equals(that.payload) &&
        this.signature.equals(that.signature) && this.to.equals(that.to) &&
  ...
  @Override
  public int hashCode() {
    return Objects.hash(nonce, gasPrice, gasLimit, to, value, payload,
                        signature, chainId, privateFor, privateFrom,
                        restriction);
  }
  ...
  public String toString() {
    ...
    sb.append(isContractCreation() ? "ContractCreation" : "MessageCall")
        .append("{");
    ...
    if (getPrivateFor().isPresent())
      sb.append("privateFor=")
          ...
          .append(", ");
    if (getPrivacyGroupId().isPresent())
      sb.append("privacyGroupId=")
          .append(getPrivacyGroupId().get())
          .append(", ");
  ...
  public static class Builder {
    ...
    public PrivateTransaction build() {
      ...
      return new PrivateTransaction(nonce, gasPrice, gasLimit,
                                    Optional.ofNullable(to), value, signature,
                                    payload, sender, chainId, privateFrom,
                                    privateFor, privacyGroupId, restriction);
    }

    public PrivateTransaction signAndBuild(final SECP256K1.KeyPair keys) {
      checkState(
          signature == null,
          "The transaction signature has already been provided to this builder");
      signature(computeSignature(keys));
      sender(Address.extract(Hash.hash(keys.getPublicKey().getEncodedBytes())));
      return build();
    }

    protected SECP256K1.Signature
    computeSignature(final SECP256K1.KeyPair keys) {
      final Bytes32 hash = computeSenderRecoveryHash(
          nonce, gasPrice, gasLimit, to, value, payload, chainId, privateFrom,
          privateFor, privacyGroupId, restriction.getBytes());
