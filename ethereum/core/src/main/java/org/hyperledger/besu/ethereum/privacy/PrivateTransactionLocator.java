...
public class PrivateTransactionLocator {
  ...
  public PrivateTransactionLocator(
      final Blockchain blockchain, final Enclave enclave,
  ...
  /**
   * Returns a private transaction with extra data (block and pmt information).
   * The private transaction is retrieved from the Enclave, either as a single
   * transaction or from an existing "blob".<br> In both cases, we are passing
   * the enclaveKey (public key of the participant) down to the Enclave, with
   * the expectation that the Enclave will only return the payload if the
   * matching private key of the enclaveKey passed as a parameter is able to be
   * used to decrypt the payload.
   *
   * <p>This enclaveKey "validation" in the Enclave makes retrieving a payload
   * multi-tenancy safe.
   *
   * @param pmtHash the hash of the PMT associated with the private transaction
   * @param enclaveKey participant public key that must match the private key
   *     used to decrypt the payload
   * @return An executed private transaction
   */
  public Optional<ExecutedPrivateTransaction>
  findByPmtHash(final Hash pmtHash, final String enclaveKey) {
    ...
    final Transaction pmt =
        blockchain.getTransactionByHash(pmtHash).orElseThrow();
   ...
   * @param enclaveKey participant public key that must match the private key
   *     used to decrypt the payload
   * @return an optional containing the private transaction, if found. Or an
   *     empty optional if the private transaction couldn't be found.
   */
  private Optional<TransactionFromEnclave>
  tryFetchingPrivateTransactionFromEnclave(final String payloadKey,
                                           final String enclaveKey) {
    return retrievePayloadFromEnclave(payloadKey, enclaveKey)
        .map(this::readPrivateTransactionFromPayload);
  }

  private Optional<ReceiveResponse>
  retrievePayloadFromEnclave(final String payloadKey, final String enclaveKey) {
    try {
      return Optional.of(enclave.receive(payloadKey, enclaveKey));
    } catch (final EnclaveClientException e) {
      // Enclave throws an exception with a 404 status code if the payload isn't
      // found
  ...
  private TransactionFromEnclave
  readPrivateTransactionFromPayload(final ReceiveResponse receiveResponse) {
    final PrivateTransaction privateTransaction;
    final BytesValueRLPInput input = new BytesValueRLPInput(
        Bytes.fromBase64String(new String(receiveResponse.getPayload(), UTF_8)),
        false);

    /*
     When using onchain privacy groups, the payload is a list with the first
     element being the private transaction RLP and the second element being the
     version. This is why we have the nextIsList() check.
    */
    try {
      input.enterList();
      if (input.nextIsList()) {
        // private transaction and version (we only read the first element in
        // the list)
    ...
    return new TransactionFromEnclave(privateTransaction,
                                      receiveResponse.getPrivacyGroupId());
  }

  private Optional<TransactionFromEnclave>
  tryFetchingTransactionFromAddBlob(final Bytes32 blockHash,
                                    final Hash expectedPmtHash,
                                    final String enclaveKey) {
    ...
    if (privacyGroupHeadBlockMapOptional.isPresent()) {
      final Set<Bytes32> mappedPrivacyGroupIds =
          privacyGroupHeadBlockMapOptional.get().keySet();
      ...
      for (final Bytes32 privacyGroupId : mappedPrivacyGroupIds) {
        ...
        if (addDataKey.isPresent()) {
          ...
          if (receiveResponse.isEmpty()) {
            LOG.warn(
                "Unable to find private transaction with payloadKey = {} on AddBlob",
                payloadKey);
            return Optional.empty();
          }

          final Bytes payload = Bytes.wrap(
              Base64.getDecoder().decode(receiveResponse.get().getPayload()));
  ...
  private static class TransactionFromEnclave {
    ...
    public String getInternalPrivacyGroupId() { return internalPrivacyGroupId; }
  }
}
