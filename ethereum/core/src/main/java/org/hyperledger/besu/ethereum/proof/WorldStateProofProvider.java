...
public class WorldStateProofProvider {
  ...
  public Optional<WorldStateProof>
  getAccountProof(final Hash worldStateRoot, final Address accountAddress,
  ...
  private SortedMap<UInt256, Proof<Bytes>>
  getStorageProofs(final StateTrieAccountValue account,
                   final List<UInt256> accountStorageKeys) {
    ...
    accountStorageKeys.forEach(
        key
        -> storageProofs.put(
            key, storageTrie.getValueWithProof(Hash.hash(key.toBytes()))));
    return storageProofs;
  }

  private MerklePatriciaTrie<Bytes32, Bytes>
  newAccountStateTrie(final Bytes32 rootHash) {
    return new StoredMerklePatriciaTrie<>(
        worldStateStorage::getAccountStateTrieNode, rootHash, b -> b, b -> b);
  }

  private MerklePatriciaTrie<Bytes32, Bytes>
  newAccountStorageTrie(final Bytes32 rootHash) {
    return new StoredMerklePatriciaTrie<>(
