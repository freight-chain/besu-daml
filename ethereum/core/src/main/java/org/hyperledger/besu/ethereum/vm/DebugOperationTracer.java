  ...
  public void traceExecution(final MessageFrame frame,
    ...
    try {
      ...
      lastFrame = new TraceFrame(
          pc, Optional.of(opcode), gasRemaining, currentGasCost,
          frame.getGasRefund(), depth, exceptionalHaltReason,
          frame.getRecipientAddress(), frame.getApparentValue(), inputData,
          outputData, stack, memory, storage, worldUpdater,
          frame.getRevertReason(), maybeRefunds,
    ...
    haltReason.ifPresent(exceptionalHaltReason -> {
      if (!traceFrames.isEmpty()) {
        ...
        final TraceFrame traceFrame = new TraceFrame(
            frame.getPC(), Optional.empty(), frame.getRemainingGas(),
            Optional.empty(), frame.getGasRefund(),
            frame.getMessageStackDepth(), Optional.of(exceptionalHaltReason),
            frame.getRecipientAddress(), frame.getValue(), frame.getInputData(),
            frame.getOutputData(), Optional.empty(), Optional.empty(),
            Optional.empty(), frame.getWorldState(), Optional.empty(),
            Optional.ofNullable(frame.getRefunds()),
            Optional.ofNullable(frame.getCode()), frame.getMaxStackSize(),
