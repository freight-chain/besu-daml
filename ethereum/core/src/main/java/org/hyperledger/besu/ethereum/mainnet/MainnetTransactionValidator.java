 ...
 *
 * <p>The {@link MainnetTransactionValidator} performs the intrinsic gas cost
 * check on the given
 * {@link Transaction}.
 */
public class MainnetTransactionValidator implements TransactionValidator {
  ...
  public MainnetTransactionValidator(final GasCalculator gasCalculator,
  ...
  public MainnetTransactionValidator(
      ...
      final boolean checkSignatureMalleability,
      final Optional<BigInteger> chainId, final Optional<EIP1559> maybeEip1559,
  ...
  validate(final Transaction transaction) {
    ...
    if (intrinsicGasCost.compareTo(Gas.of(transaction.getGasLimit())) > 0) {
      return ValidationResult.invalid(
          ...
          String.format("intrinsic gas cost %s exceeds gas limit %s",
  ...
  validateForSender(final Transaction transaction, final Account sender,
    ...
    if (transaction.getNonce() < senderNonce) {
      return ValidationResult.invalid(
          ...
          String.format("transaction nonce %s below sender account nonce %s",
                        transaction.getNonce(), senderNonce));
    }

    if (!validationParams.isAllowFutureNonce() &&
    ...
    if (!isSenderAllowed(transaction, validationParams)) {
      return ValidationResult.invalid(
          ...
          String.format("Sender %s is not on the Account Allowlist",
  ...
  public ValidationResult<TransactionInvalidReason>
  validateTransactionSignature(final Transaction transaction) {
    ...
    if (disallowSignatureMalleability &&
      ...
      return ValidationResult.invalid(
          ...
          String.format("Signature s value should be less than %s, but got %s",
                        SECP256K1.HALF_CURVE_ORDER, signature.getS()));
    }

    // org.bouncycastle.math.ec.ECCurve.AbstractFp.decompressPoint throws an
    // IllegalArgumentException for "Invalid point compression" for bad
    // signatures.
    try {
      transaction.getSender();
  ...
  private boolean
  isSenderAllowed(final Transaction transaction,
                  final TransactionValidationParams validationParams) {
    if (validationParams.checkLocalPermissions() ||
        validationParams.checkOnchainPermissions()) {
      return transactionFilter
          .map(c
               -> c.permitted(transaction,
                              validationParams.checkOnchainPermissions()))
          .orElse(true);
    } else {
      return true;
