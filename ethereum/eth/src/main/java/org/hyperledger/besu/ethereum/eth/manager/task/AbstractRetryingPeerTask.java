...
/**
 * A task that will retry a fixed number of times before completing the
 * associated CompletableFuture exceptionally with a new {@link
 * MaxRetriesReachedException}. If the future returned from {@link
 * #executePeerTask(Optional)} is complete with a non-empty list the retry
 * counter is reset.
 *
 * @param <T> The type as a typed list that the peer task can get partial or
 *     full results in.
 */
public abstract class AbstractRetryingPeerTask<T> extends AbstractEthTask<T> {
  ...
  /**
   * @param ethContext The context of the current Eth network we are attached
   *     to.
   * @param maxRetries Maximum number of retries to accept before completing
   *     exceptionally.
   * @param isEmptyResponse Test if the response received was empty.
   * @param metricsSystem The metrics system used to measure task.
   */
  protected AbstractRetryingPeerTask(final EthContext ethContext,
  ...
  executePeerTask(Optional<EthPeer> assignedPeer);
  ...
  private void handleTaskError(final Throwable error) {
    ...
    if (cause instanceof NoAvailablePeersException) {
      LOG.info("No peers available, waiting for peers: {}",
               ethContext.getEthPeers().peerCount());
      // Wait for new peer to connect
      final WaitForPeerTask waitTask =
    ...
    LOG.debug("Retrying after recoverable failure from peer task {}: {}",
              this.getClass().getSimpleName(), cause.getMessage());
    // Wait before retrying on failure
    executeSubTask(()
                       -> ethContext.getScheduler().scheduleFutureTask(
                           this::executeTaskTimed, Duration.ofSeconds(1)));
  }
  ...
  private boolean isRetryableError(final Throwable error) {
    ...
    return error instanceof TimeoutException ||
        (!assignedPeer.isPresent() && isPeerError);
  }
