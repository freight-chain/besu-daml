...
/**
 * Manages underlying UPnP library "jupnp" and provides abstractions for
 * asynchronously interacting with the NAT environment through UPnP.
 */
public class UpnpNatManager extends AbstractNatManager {
  ...
  // internally-managed future. external queries for IP addresses will be
  // copy()ed from this.
  private final CompletableFuture<String> externalIpQueryFuture =
      new CompletableFuture<>();

  private final Map<String, CompletableFuture<RemoteService>>
  ...
  public UpnpNatManager() {
    ...
    // Workaround for an issue in the jupnp library: the ExecutorService used
    // misconfigures its ThreadPoolExecutor, causing it to only launch a single
    // thread. This prevents any work from getting done (effectively a
    // deadlock). The issue is fixed here:
  ...
  UpnpNatManager(final UpnpService service) {
    ...
    recognizedServices.put(SERVICE_TYPE_WAN_IP_CONNECTION,
                           new CompletableFuture<>());

    // registry listener to observe new devices and look for specific services
    registryListener = new BesuUpnpRegistryListener() {
      @Override
      public void remoteDeviceAdded(final Registry registry,
                                    final RemoteDevice device) {
        LOG.debug("UPnP Device discovered: " +
  ...
  public synchronized void doStop() {
    ...
    try {
      ...
      LOG.warn(
          "Caught exception while trying to release port forwards, ignoring",
          e);
    }

    for (CompletableFuture<RemoteService> future :
         recognizedServices.values()) {
      future.cancel(true);
    }
   ...
   * @return a CompletableFuture that will provide the desired RemoteService
   *     when completed
   */
  private synchronized CompletableFuture<RemoteService>
  getService(final String type) {
   ...
   *
   * @return a CompletableFuture that will provide the WANIPConnection
   *     RemoteService when completed
   */
  @VisibleForTesting
  ...
  /**
   * Returns a CompletableFuture that will wait for the given service type to be
   * discovered. No new query will be performed, and if the service has already
   * been discovered, the future will complete in the very near future.
   *
   * @param serviceType is the service type to wait to be discovered.
   * @return future that will return the desired service once it is discovered,
   *     or null if the future is cancelled.
   */
  private synchronized CompletableFuture<RemoteService>
  discoverService(final String serviceType) {
   ...
   *
   * @return A CompletableFuture that can be used to query the result (or
   *     error).
   */
  @Override
   ...
   *
   * @return The known port mappings wrapped in a {@link
   *     java.util.concurrent.Future}.
   */
  @Override
  public CompletableFuture<List<NatPortMapping>> getPortMappings() {
    checkState(isStarted(),
               "Cannot call getPortMappings() when in stopped state");
   ...
   * Sends a UPnP request to the discovered IGD for the external ip address.
   *
   * <p>Note that this is not synchronized, as it is expected to be called
   * within an already-synchronized context ({@link #start()}).
   */
  private void initiateExternalIpQuery() {
    discoverService(SERVICE_TYPE_WAN_IP_CONNECTION).thenAccept(service -> {
      ...
      GetExternalIP callback = new GetExternalIP(service) {
        /**
         * Override the success(ActionInvocation) version of success so that we
         * can take a peek at the network interface that we discovered this on.
         *
        ...
        public void success(final ActionInvocation invocation) {
          ...
          discoveredOnLocalAddress = Optional.of(
        ...
        protected void success(final String result) {
          ...
          LOG.info("External IP address {} detected for internal address {}",
                   result, discoveredOnLocalAddress.get());

          externalIpQueryFuture.complete(result);
        }

        /**
         * Because the underlying jupnp library omits generics info in this
         * method signature, we must too when we override it.
         */
        @Override
        @SuppressWarnings("rawtypes")
        public void failure(final ActionInvocation invocation,
                            final UpnpResponse operation, final String msg) {
  ...
  /**
   * Convenience function to call {@link #requestPortForward(PortMapping)} with
   * the following defaults:
   *
   * <p>enabled: true leaseDurationSeconds: 0 (indefinite) remoteHost: null
   * internalClient: the local address used to discover gateway
   *
   * <p>In addition, port is used for both internal and external port values.
   *
   * @param port is the port to be used for both internal and external port
   *     values
   ...
   */
  public void requestPortForward(final int port, final NetworkProtocol protocol,
                                 final NatServiceType serviceType) {
    checkState(isStarted(),
               "Cannot call requestPortForward() when in stopped state");
    checkArgument(port != 0, "Cannot map to internal port zero.");
    this.requestPortForward(new PortMapping(
        true, new UnsignedIntegerFourBytes(0), null,
        new UnsignedIntegerTwoBytes(port), new UnsignedIntegerTwoBytes(port),
        null, toJupnpProtocol(protocol), serviceType.getValue()));
  }

  /**
   * Sends a UPnP request to the discovered IGD to request a port forward.
   *
   * @param portMapping is a portMapping object describing the desired port
   *     mapping parameters.
   * @return A CompletableFuture that can be used to query the result (or
   *     error).
   */
  private CompletableFuture<Void>
  requestPortForward(final PortMapping portMapping) {

    CompletableFuture<Void> upnpQueryFuture = new CompletableFuture<>();

    return externalIpQueryFuture.thenCompose(address -> {
      // note that this future is a dependency of externalIpQueryFuture, so it
      // must be completed by now
      RemoteService service = getService(SERVICE_TYPE_WAN_IP_CONNECTION).join();

      // at this point, we should have the local address we discovered the IGD
      // on, so we can prime the NewInternalClient field if it was omitted
      ...
      PortMappingAdd callback = new PortMappingAdd(service, portMapping) {
        ...
        @SuppressWarnings("rawtypes")
        public void success(final ActionInvocation invocation) {
          LOG.info("Port forward request for {} {} -> {} succeeded.",
                   portMapping.getProtocol(), portMapping.getInternalPort(),
          ...
          synchronized (forwardedPorts) {
            ...
            final NatPortMapping natPortMapping = new NatPortMapping(
                natServiceType,
                NetworkProtocol.valueOf(portMapping.getProtocol().name()),
                portMapping.getInternalClient(), portMapping.getRemoteHost(),
        ...
        public void failure(final ActionInvocation invocation,
                            final UpnpResponse operation, final String msg) {
          LOG.warn("Port forward request for {} {} -> {} failed: {}",
                   portMapping.getProtocol(), portMapping.getInternalPort(),
                   portMapping.getExternalPort(), msg);
          upnpQueryFuture.completeExceptionally(new Exception(msg));
        }
      };

      LOG.info("Requesting port forward for {} {} -> {}",
               portMapping.getProtocol(), portMapping.getInternalPort(),
               portMapping.getExternalPort());
   ...
   *
   * <p>Note that this is not synchronized, as it is expected to be called
   * within an already-synchronized context ({@link #stop()}).
   *
   * @return A CompletableFuture that will complete when all port forward
   *     requests have been made
   */
  private CompletableFuture<Void> releaseAllPortForwards() {
    // if we haven't observed the WANIPConnection service yet, we should have no
    // port forwards to release
    ...
    while (!done) {
      ...
      LOG.info("Releasing port forward for {} {} -> {}",
               portMapping.getProtocol(), portMapping.getInternalPort(),
               portMapping.getExternalPort());

      CompletableFuture<Void> future = new CompletableFuture<>();

      PortMappingDelete callback = new PortMappingDelete(
          service, toJupnpPortMapping(portMapping)) {
        /**
         * Because the underlying jupnp library omits generics info in this
         * method signature, we must too when we override it.
         */
        ...
        @SuppressWarnings("rawtypes")
        public void success(final ActionInvocation invocation) {
          LOG.info("Port forward {} {} -> {} removed successfully.",
                   portMapping.getProtocol(), portMapping.getInternalPort(),
                   portMapping.getExternalPort());
    ...
    // return a future that completes succeessfully only when each of our port
    // delete requests complete
    ...
    for (RemoteService service : device.getServices()) {
      ...
      if (serviceTypes.contains(serviceType)) {
        synchronized (this) {
          ...
          CompletableFuture<RemoteService> existingFuture =
              recognizedServices.get(serviceType);
  ...
  private PortMapping toJupnpPortMapping(final NatPortMapping natPortMapping) {
    return new PortMapping(
        ...
        new UnsignedIntegerTwoBytes(natPortMapping.getInternalPort()), null,
