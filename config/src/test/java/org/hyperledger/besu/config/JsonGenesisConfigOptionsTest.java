...
public class JsonGenesisConfigOptionsTest {
  ...
  private ObjectNode loadCompleteDataSet() {
    try {
      final String configText = Resources.toString(
          Resources.getResource("valid_config_with_custom_forks.json"),
  ...
  private ObjectNode loadConfigWithNoIbft2Forks() {
    final ObjectNode configNode = loadCompleteDataSet();
    final ObjectNode transitionsNode =
        JsonUtil.getObjectNode(configNode, "transitions").get();
  ...
  private ObjectNode loadConfigWithAnIbft2ForkWithMissingValidators() {
    ...
    final ObjectNode transitionsNode =
        JsonUtil.getObjectNode(configNode, "transitions").get();
    final ArrayNode ibftNode =
        JsonUtil.getArrayNode(transitionsNode, "ibft2").get();
  ...
  public void transitionsDecodesCorrectlyFromFile() {
    ...
    assertThat(configOptions.getTransitions().getIbftForks().size())
        .isEqualTo(2);
    assertThat(
        configOptions.getTransitions().getIbftForks().get(0).getForkBlock())
        .isEqualTo(20);
    assertThat(
        configOptions.getTransitions().getIbftForks().get(0).getValidators())
        .isNotEmpty();
    assertThat(configOptions.getTransitions()
                   .getIbftForks()
                   .get(0)
                   .getValidators()
                   .get())
    ...
    assertThat(
        configOptions.getTransitions().getIbftForks().get(1).getForkBlock())
        .isEqualTo(25);
    assertThat(
        configOptions.getTransitions().getIbftForks().get(1).getValidators())
        .isNotEmpty();
    assertThat(configOptions.getTransitions()
                   .getIbftForks()
                   .get(1)
  ...
  public void configWithAnIbftWithNoValidatorsListedIsValid() {
    final ObjectNode configNode =
        loadConfigWithAnIbft2ForkWithMissingValidators();
  ...
  public void configWithValidIbftBlockRewardIsParsable() {
    ...
    final JsonGenesisConfigOptions configOptions =
        JsonGenesisConfigOptions.fromJsonObject(configNode);
    assertThat(configOptions.getIbft2ConfigOptions().getMiningBeneficiary())
        .isNotEmpty();
    assertThat(
        configOptions.getIbft2ConfigOptions().getMiningBeneficiary().get())
        .isEqualTo("0x1234567890123456789012345678901234567890");
    assertThat(configOptions.getIbft2ConfigOptions().getBlockRewardWei())
        .isEqualTo(21);
  }
