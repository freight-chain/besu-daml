...
public class MetricsHttpServiceTest {
  ...
  private static MetricsHttpService
  createMetricsHttpService(final MetricsConfiguration config) {
    return new MetricsHttpService(vertx, config,
                                  PrometheusMetricsSystem.init(config));
  }
  ...
  private static MetricsConfiguration.Builder createMetricsConfigBuilder() {
    return MetricsConfiguration.builder().enabled(true).port(0).hostsAllowlist(
        singletonList("*"));
  }
  ...
  public void getSocketAddressWhenStoppedIsEmpty() {
    ...
    assertThat(new InetSocketAddress("0.0.0.0", 0))
        .isEqualTo(service.socketAddress());
  }

  @Test
  public void getSocketAddressWhenBindingToAllInterfaces() {
    final MetricsConfiguration config =
        createMetricsConfigBuilder().host("0.0.0.0").build();
  ...
  public void metricsArePresent() throws Exception {
    final Request metricsRequest =
        new Request.Builder().url(baseUrl + "/metrics").build();
  ...
  public void metricsArePresentWhenFiltered() throws Exception {
    final Request metricsRequest =
        new Request.Builder()
            .url(baseUrl + "/metrics?name[]=jvm_threads_deadlocked")
            .build();
    try (final Response resp = client.newCall(metricsRequest).execute()) {
      assertThat(resp.code()).isEqualTo(200);
      // Check general format of result, it maps to java.util.Properties
  ...
  public void metricsAreAbsentWhenFiltered() throws Exception {
    final Request metricsRequest =
        new Request.Builder()
            .url(baseUrl + "/metrics?name[]=does_not_exist")
            .build();
    try (final Response resp = client.newCall(metricsRequest).execute()) {
      assertThat(resp.code()).isEqualTo(200);
      // Check general format of result, it maps to java.util.Properties
