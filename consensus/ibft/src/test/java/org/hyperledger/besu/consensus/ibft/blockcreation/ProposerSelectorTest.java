...
public class ProposerSelectorTest {
  ...
  private Blockchain
  createMockedBlockChainWithHeadOf(final long blockNumber,
    ...
    when(voteTallyCache.getVoteTallyAfterBlock(any()))
        .thenReturn(new VoteTally(validators));

    final BlockHeaderTestFixture headerBuilderFixture =
    ...
    when(blockchain.getBlockHeader(anyLong()))
        .thenReturn(Optional.of(prevBlockHeader));

    return blockchain;
  }

  /**
   * This creates a list of validators, with the a number of validators above
   * and below the local address. The returned list is sorted.
   *
   * @param localAddr The address of the node which signed the parent block
   * @param countLower The number of validators which have a higher address than
   *     localAddr
   * @param countHigher The number of validators which have a lower address than
   *     localAddr
   * @return A sorted list of validators which matches parameters (including the
   *     localAddr).
   */
  private List<Address> createValidatorList(final Address localAddr,
                                            final int countLower,
                                            final int countHigher) {
    ...
    for (int i = 0; i < countLower; i++) {
      result.add(AddressHelpers.calculateAddressWithRespectTo(localAddr,
                                                              i - countLower));
    }

    for (int i = 0; i < countHigher; i++) {
      result.add(
          AddressHelpers.calculateAddressWithRespectTo(localAddr, i + 1));
    }
  ...
  public void roundRobinChangesProposerOnRoundZeroOfNextBlock() {
    ...
    final Address localAddr =
        AddressHelpers.ofValue(10); // arbitrarily selected

    final List<Address> validatorList = createValidatorList(localAddr, 0, 4);
    final Blockchain blockchain = createMockedBlockChainWithHeadOf(
        PREV_BLOCK_NUMBER, localAddr, validatorList);
    ...
    final ConsensusRoundIdentifier roundId =
        new ConsensusRoundIdentifier(PREV_BLOCK_NUMBER + 1, 0);
  ...
  public void stickyProposerChangesOnSubsequentRoundsAtSameBlockHeight() {
    final long PREV_BLOCK_NUMBER = 2;
    ConsensusRoundIdentifier roundId =
        new ConsensusRoundIdentifier(PREV_BLOCK_NUMBER + 1, 0);
    ...
    assertThat(uut.selectProposerForRound(roundId))
        .isEqualTo(validatorList.get(0));
  ...
  public void whenProposerSelfRemovesSelectsNextProposerInLineEvenWhenSticky() {
    ...
    // LocalAddr will be in index 2 - the next proposer will also be in 2 (as
    // prev proposer is removed)
    ...
    assertThat(uut.selectProposerForRound(roundId))
        .isEqualTo(validatorList.get(2));
  }

  @Test
  public void
  whenProposerSelfRemovesSelectsNextProposerInLineEvenWhenRoundRobin() {
    final long PREV_BLOCK_NUMBER = 2;
    final ConsensusRoundIdentifier roundId =
        new ConsensusRoundIdentifier(PREV_BLOCK_NUMBER + 1, 0);
  ...
  @Test
  public void
  proposerSelfRemovesAndHasHighestAddressNewProposerIsFirstInList() {
    final long PREV_BLOCK_NUMBER = 2;
    final ConsensusRoundIdentifier roundId =
        new ConsensusRoundIdentifier(PREV_BLOCK_NUMBER + 1, 0);
