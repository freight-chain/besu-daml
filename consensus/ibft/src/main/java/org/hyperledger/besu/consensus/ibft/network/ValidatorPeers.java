...
/**
 * Responsible for tracking the network peers which have a connection to this
 * node, then multicasting packets to ONLY the peers which have been identified
 * as being validators.
 */
public class ValidatorPeers
  ...
  // It's possible for multiple connections between peers to exist for brief
  // periods, so map each address to a set of connections
  private final Map<Address, Set<PeerConnection>> connectionsByAddress =
  ...
  public void remove(final PeerConnection removedConnection) {
    ...
    final Set<PeerConnection> connections =
        connectionsByAddress.get(peerAddress);
    if (connections == null) {
      return;
    }
  ...
  @Override
  public void send(final MessageData message,
                   final Collection<Address> denylist) {
  ...
  private void
  sendMessageToSpecificAddresses(final Collection<Address> recipients,
                                 final MessageData message) {
    LOG.trace("Sending message to peers messageCode={} recipients={}",
              message.getCode(), recipients);
    recipients.stream()
        ...
        .forEach(connection -> {
          try {
            ...
            LOG.trace(
                ...
                connection.getRemoteAddress(), connection.getPeerInfo());
          }
        });
  }
