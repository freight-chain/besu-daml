...
public class IbftProtocolSchedule {
  ...
  create(final GenesisConfigOptions config,
    ...
    return new ProtocolScheduleBuilder(
               ...
               builder
               -> applyIbftChanges(config.getIbft2ConfigOptions(), builder),
  ...
  applyIbftChanges(final IbftConfigOptions ibftConfig,
                   final ProtocolSpecBuilder builder) {

    if (ibftConfig.getEpochLength() <= 0) {
      throw new IllegalArgumentException(
          "Epoch length in config must be greater than zero");
    }

    if (ibftConfig.getBlockRewardWei().signum() < 0) {
      throw new IllegalArgumentException(
          "Ibft2 Block reward in config cannot be negative");
    }

    builder
        .blockHeaderValidatorBuilder(
            ibftBlockHeaderValidator(ibftConfig.getBlockPeriodSeconds()))
    ...
    if (ibftConfig.getMiningBeneficiary().isPresent()) {
      ...
      try {
        // Precalculate beneficiary to ensure string is valid now, rather than
        // on lambda execution.
        miningBeneficiary =
            Address.fromHexString(ibftConfig.getMiningBeneficiary().get());
