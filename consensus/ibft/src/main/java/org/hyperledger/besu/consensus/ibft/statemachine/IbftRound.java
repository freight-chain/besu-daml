...
public class IbftRound {
  ...
  private void updateStateWithProposalAndTransmit(
    ...
    final Proposal proposal = messageFactory.createProposal(
        getRoundIdentifier(), block, roundChangeCertificate);
  ...
  private boolean updateStateWithProposedBlock(final Proposal msg) {
    ...
    if (blockAccepted) {
      ...
      final Commit localCommitMessage = messageFactory.createCommit(
          roundState.getRoundIdentifier(), msg.getBlock().getHash(),
